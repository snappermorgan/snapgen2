...
class Forms3rdpartyDynamicFields {
	...
	public function Forms3rdpartyDynamicFields() {
		...
		add_action(self::B.'_service', array(&$this, 'adjust_response'), 10, 2);
	}
	...
	const REPLACE_PARAM_PREFIX = "##REPLACE:{";
	const IP = "##IP##";
	const PREFIX_LEN = 7; // the length of GETPARAM_PREFIX
	const REPLACE_PREFIX_LEN = 11;
	/**
	 * placeholder for response attachments
	 */
	private $_dynamic_attach;

	public function post_filter($post, $service, $form, $sid=0, $submission = false) {
	...
	public function is_replace($value) {
		...
		switch($value) {
			...
			default:
				...
				elseif(0 === strpos($value, self::REPLACE_PARAM_PREFIX)) return true;
				break;
		} // switch $value
	...
	public function replace($value, $post = false, $submission=false) {
		// known placeholders
		switch($value) {
			case self::TIMESTAMP:
				return time();
			...
			default:
				...
				}elseif(0 === strpos($value, self::REPLACE_PARAM_PREFIX)){
					$template = substr($value,self::REPLACE_PREFIX_LEN, -3);
					
					$value = $this->bind_to_template($submission,$template);
					return $value;
				}
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td class="dyn-field"><code><?php echo $t ?></code></td>
							<td><?php echo $this->replace($t); ?></td>
							<td><?php _e('IP address of submitter', $P) ?></td>
						</tr>
						<tr>
						...
						<tr>
							<?php $t = self::REPLACE_PARAM_PREFIX; ?>
							<td class="dyn-field" valign="top"><code><?php echo $t ?>%%input_1%% %%input_2%% is %%input_12%% years old}##</code></td>
							<td></td>
							<td><?php _e('To create a new value that is a combination of submission values and static text, wrap the name of the field in <code>%%</code>. 
							<p>Example: To create the phrase "John Smith is 32 years old" with the field submission values, use the placeholder: <p> <code>##REPLACE:{%%input_1%% %%input_2%% is %%input_12%% years old.}##</code>', $P) ?></td>
						</tr>
					</tbody>
				</table>
