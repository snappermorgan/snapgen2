...
class Forms3rdPartyIntegration {
	...
	function add_admin_headers() {
		...
		wp_enqueue_script('zero', plugins_url('ZeroClipboard.js', __FILE__));
	...
	public function form_select_input($forms, $eid, $selected) {
		?>
		<select class="multiple forms-select-input" multiple="multiple" id="forms-<?php echo $eid?>" name="<?php echo $this->N?>[<?php echo $eid?>][forms][]">
<?php
foreach ($forms as $f) {
			$form_id = $f['id'];
...
<?php
//let's convert the form field info to a json array suitable for printing.
		$form_fields_json = json_encode($forms);
		?>
	    <script type="text/javascript">
	    form_fields_json = <?php echo $form_fields_json;?>;
	    </script>
<?php
	...
	function before_send($form) {
		...
		foreach ($services as $sid => $service):
			...
			if (!isset($service['forms']) || empty($service['forms'])) {
				continue;
			}
			// nothing provided
			...
			$submission = apply_filters($this->N('alter_submission'), $service, $submission);
