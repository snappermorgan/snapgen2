...
*/
	...
	public function select_forms($forms){
		...
		foreach($plugin_forms as $f) {
			$form = array(
				...
				, 'title' => sprintf('%s', $this->GET_FORM_LIST_TITLE($f))
				, 'form_object' => $this->GET_FORM_FIELDS($this->GET_FORM_LIST_ID($f))
			);
			// add to list
			$forms []= $form;
		}
	...
	public function use_form($result, $form, $service_id, $service_forms) {
		...
		if( in_array($this->FORM_ID_PREFIX() . $this->GET_FORM_ID($form), $service_forms) ) {
			...
			add_filter(Forms3rdPartyIntegration::$instance->N('remote_success'), array(&$this, 'remote_success'), 10, 4);
	 ...
	 * @return void                   n/a
	 */
	public function remote_success($form, $callback_results, $service, $append = false) {
	...
	public function remote_failure($form, $debug, $service, $post, $response){
		...
		//notify admin
		// added new switch - cfm
		if(isset($service['notify-admin']) && !empty($service['notify-admin'])) {
