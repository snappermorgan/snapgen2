(function($){
	var lib = {
		...
		action: function(e) {
			...
			switch(action) {
				case 'clone': lib.clone($target, after,false); break;
				case 'remove': lib.remove($target, after); break;
				case 'toggle':
				case 'toggle-sibling': lib.toggle($target, after, action); break;
				case 'duplicate': lib.clone($target,after, true)
			}

		}//--	fn	action
		,
		clone: function($target, after, dupe) {
jQuery(".forms-select-input").select2("destroy");			
var	$clone = $target.clone();	//clone the target so we can add it later
			
			// perform requested post-operation
			lib.afterClone[after]($target, $clone, lib.id(), dupe);
			...
			$target.after( $clone );
jQuery(".forms-select-input").select2({closeOnSelect:false,width:"500"});
		}//--	fn	clone
		,
		...
		afterClone: {
			...
			,
			metabox: function($target, $clone, newid,dupe) {
				...
				lib.updateClonedRow(newid, $clone, /(\[)([\d])/, dupe);
			}//--	fn	afterClone.metabox
		}//--	afterClone
		,
		updateClonedRow: function(newid, $clone, regex,dupe) {
			//reset clone values and update indices
			$clone.find('input:not(".select2-input"),select').each(function(i, o){
				var $o = $(o)
					, id = $o.attr('id').split('-')
					, name = $o.attr('name')
					;
	...
	$(function() {
		...
		var $postbox = $plugin.find('div.postbox').addClass('collapsed')
		$postbox.each(function(i,o) {
				$(o).find('h3').not(".inner")
					.prepend('<span class="s">[' + ($(o).data('icon') || '+') + ']</span> ')
					.addClass('actn')
					.data('actn', "toggle")
					.data('rel', ".postbox")
					;

					//find inner h3 because im a hack
					$(o).find('h3.inner')
					.data('actn', "toggle")
					.data('rel', ".postbox")
					;
