	...
	*/
    ...
    function short_tag_wpv_view_form( $atts ) {
	    ...
	    if ( isset( $view_settings['filter_meta_html'] ) ) {
			...
			// TODO only add this if posting to the current page:
			// self or target_id = current page
...
';
    ...
    */
    ...
    function convert_ids_to_names_in_settings( $settings ) {
		...
		// TODO a lot of redundant code is here; maybe create few smaller methods for repeating actions?
        ...
        if ( isset( $settings['parent_id'] ) && $settings['parent_id'] != '' ) {
            $parent_name = $wpdb->get_var( $wpdb->prepare(
		...
		*/
		...
		foreach ( $settings as $set_key => $set_value ) {
			...
			} elseif ( strpos( $set_key, 'taxonomy-' ) === 0
    ...
    function convert_ids_to_names_in_filters( $id_list ) {
        ...
        foreach ( $id_array as $id_post ) {
			$name = $wpdb->get_var( $wpdb->prepare( "SELECT post_name FROM {$wpdb->posts} WHERE ID = %d", $id_post ) );
			if (!is_null( $name ) ) {
				$post_names[ 'post-' . $counter ] = $name;
			}
			$counter++;
        }
        return $post_names;
    ...
    function convert_names_to_ids_in_settings( $settings ) {
        ...
        if ( isset( $settings['taxonomy_types'] ) ) {
            ...
            } elseif ( isset( $settings['taxonomy_type']['taxonomy_type'] ) ) {
        ...
        if ( isset( $settings['roles_types'] ) ) {
            ...
            } elseif ( isset( $settings['roles_type']['roles_type'] ) ) {
		...
		if ( isset( $settings['author_name'] ) && !empty( $settings['author_name'] ) ) {
			$new_author = '';
			$new_author = $wpdb->get_var( $wpdb->prepare(
    ...
    function convert_names_to_ids_in_filters( $names_array ) {
		...
		foreach ( $names_array as $post_name ) {
			$found_post = $wpdb->get_var( $wpdb->prepare(
					"SELECT ID FROM {$wpdb->posts} WHERE post_name = %s",
					$post_name ) );
    ...
    function convert_names_to_ids_in_layout_settings( $settings ) {
        ...
        if ( isset( $settings['fields'] ) ) {
            foreach($settings['fields'] as $key => $value) {
                if ( substr( $key, 0, 5 ) == 'name_' ) {
					/* not needed anymore as we are storing the name anyway on export
                    if (substr($value, 0, 13) == 'wpv-post-body') {
		...
		if ( isset( $settings['included_ct_ids'] ) ) {
			...
			if ( count( $templates ) > 0 ) {
				$attached_templates = count( $templates );
				for ( $i = 0; $i < $attached_templates; $i++ ) {
					$template_found = $wpdb->get_var( $wpdb->prepare(
							"SELECT ID FROM {$wpdb->posts} WHERE post_title = %s AND post_type = 'view-template'",
							$templates[$i] ) );
	...
	*/
	...
	function wpv_admin_enqueue_scripts( $hook ) {
		...
		wp_register_script( 'views-utils-script' , WPV_URL_EMBEDDED . '/res/js/lib/utils.js', array('jquery','toolset-colorbox', 'select2'), WPV_VERSION);
 ...
 *
 * @param array $exclude_tax_slugs The slugs to be excluded.
 *
 */
