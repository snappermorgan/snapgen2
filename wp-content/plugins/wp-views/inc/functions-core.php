...
function _wpv_get_all_views($view_query_mode) {
	...
	$q = ('SELECT ID, post_title FROM ' . $wpdb->prefix . 'posts WHERE post_status="publish" AND post_type="view"');
...
function wpv_create_view( $args ) {
	...
	if ( 0 != $id ) {
		...
		switch ($args['settings']["view_purpose"]) {
			...
			case 'bootstrap-grid': // if the purpose is from Layouts plugin, create a CT and add it to the View layout and to the list of connected CT. Create the BS grid
				// TODO review this all: this is not a real purpose
				// WARNING this might be used in the Views post loop cell type too (for archive loops)
				...
				//$view_normal_layout_defaults['layout_meta_html'] = wpv_create_bootstrap_meta_html($args['cols'],$ct_post->post_name,$view_normal_layout_defaults['layout_meta_html']);
