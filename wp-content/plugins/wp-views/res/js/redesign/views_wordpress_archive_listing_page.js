jQuery(document).ready(function($) {
	...
	$(document).on('click','.js-remove-archive-permanent', function (e) {
		...
		$.ajax({
			...
			success:function(response){
				if ( (typeof(response) !== 'undefined') && (response == data.id)) {
					// TODO could we use url_params directly?
					var newParams = decodeURIParams(url_page);
					newParams['deleted'] = 1;
					navigateWithURIParams(newParams);
				} else {
					console.log( "Error: AJAX returned ", response );
				}
			},
