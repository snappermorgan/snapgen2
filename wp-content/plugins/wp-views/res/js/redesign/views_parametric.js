				...
				jQuery.post(ajaxurl, params, function(response) {
					...
					if ( (typeof(response) !== 'undefined') ) {
						...
						if ( decoded_response.success === params.id ) {
							...
							jQuery( document ).trigger( 'js_event_wpv_query_filter_created', [ 'all' ] );
						}
					} else {
		...
		return this.each(function ()
		{
			...
			var WPV_ParametricSearchButton = function()
			{
				...
				self.handle_flags = function() {
					button.removeClass( 'wpv-button-flagged' ).find( '.js-ps-button-complete, .js-ps-button-filter-missing' ).hide();
					if( self.is_button_disabled() )
					{
						button.prop('disabled','disabled').attr( 'title', title_complete ).addClass( 'wpv-button-flagged' ).find( '.js-ps-button-complete' ).show();
					}
					else
					{
						button.prop('disabled',false).attr( 'title', title );
						if ( self.has_search( self.get_text_area_content() ) && !self.has_filter() ) {
							button.attr( 'title', title_filter_missing ).addClass( 'wpv-button-flagged' ).find( '.js-ps-button-filter-missing' ).show();
						}
						self.create_search();
					}
				};
				...
				self.create_filter = function( callback, args )
				{
					var params = {
						...
						id: WPV_Parametric.view_id,
						filter_options: 'filter_by_search=1&post_search_value=&search_mode%5B%5D=manual&post_search_content=full_content',
				...
				self.handle_flags = function()
				{
					...
					if (
						...
						if( self.has_spinner( self.get_text_area_content() ) ) {
							button.attr('title',title_complete).addClass( 'wpv-button-flagged' ).find( '.js-ps-button-flag' ).show();
						} else {
							button.attr('title',title).removeClass( 'wpv-button-flagged' ).find( '.js-ps-button-flag' ).hide();
						}
					} else {
						button.prop('disabled','disabled').attr('title',title_useless).removeClass( 'wpv-button-flagged' ).find( '.js-ps-button-flag' ).hide();
					}
				};
			...
			var WPV_ParametricResetButton = function()
			{
				...
				self.handle_flags = function()
				{
					...
					{
						button.attr('title',title_complete).addClass( 'wpv-button-flagged' ).find( '.js-ps-button-complete' ).show();
					}
					else
					{
						button.attr('title',title).removeClass( 'wpv-button-flagged' ).find( '.js-ps-button-complete' ).hide();
					}
				};
			...
			var WPV_ParametricSubmitButton = function()
			{
				...
				self.handle_flags = function()
				{
					...
					if ( update_mode == 'enable' ) {
						button.prop('disabled','disabled').find( '.js-ps-button-irrelevant' ).show();
						button.find( '.js-ps-button-incomplete, .js-ps-button-complete' ).hide();
						if( self.has_submit( self.get_text_area_content() ) ) {
							button.attr('title',title_irrelevant_but_added);
						} else {
							button.attr('title',title_irrelevant);
						}
					} else {
						button.prop('disabled',false).find( '.js-ps-button-irrelevant, .js-ps-button-incomplete, .js-ps-button-complete' ).hide();
						if( self.has_submit( self.get_text_area_content() ) ) {
							button.attr('title',title_complete).find( '.js-ps-button-complete' ).show();
						} else {
							button.attr('title',title_incomplete).find( '.js-ps-button-incomplete' ).show();
						}
						self.create_dialog();
					}
				}
