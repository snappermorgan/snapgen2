 ...
 * 
 */
;(function(f,s,q){function t(b){return b.replace(/\$/g,"$$$$")}function n(b,a,c){var e,d;e=b.attr("data-dk-dropdown-value");b=b.text();d=a.data("dropkick");$select=d.$select;$select.val(e);a.find(".dk_label").text(b);c=c||!1;d.settings.change&&!c?d.settings.change.call($select,e,b):c||$select.trigger("change")}function i(b,a){a.find(".dk_option_current").removeClass("dk_option_current");b.addClass("dk_option_current");var c=b.prevAll("li").outerHeight()*b.prevAll("li").length;a.find(".dk_options_inner").animate({scrollTop:c+
"px"},0)}function l(b){b.data("dropkick");b.find(".dk_options").css({top:b.find(".dk_toggle").outerHeight()-1});b.toggleClass("dk_open")}function u(b,a){var c,e=[],d;c=b.replace("{{ id }}",a.id);c=c.replace("{{ label }}",a.label);c=c.replace("{{ tabindex }}",a.tabindex);if(a.options&&a.options.length)for(var j=0,i=a.options.length;j<i;j++){var h=f(a.options[j]),g=v,g=g.replace("{{ value }}",h.val()),g=g.replace("{{ current }}",o(h.val())===a.value?"dk_option_current":""),g=g.replace("{{ text }}",
t(h.text()));e[e.length]=g}return d=f(c),d.find(".dk_options_inner").html(e.join("")),d}function o(b){return f.trim(b).length>0?b:!1}var r=!1;f.browser.msie&&f.browser.version.substr(0,1)<7?r=!0:q.documentElement.className+=" dk_fouc";var k={},m=[],p={left:37,up:38,right:39,down:40,enter:13},v='<li class="{{ current }}"><a data-dk-dropdown-value="{{ value }}">{{ text }}</a></li>',w={startSpeed:1E3,theme:!1,change:!1};k.init=function(b){return b=f.extend({},w,b),this.each(function(){var a=f(this),
c=a.find(":selected").first(),e=a.find("option"),d=a.data("dropkick")||{},j=a.attr("id")||a.attr("name"),i=b.width||a.outerWidth(),h=a.attr("tabindex")?a.attr("tabindex"):"",g=!1;if(d.id)return a;d.settings=b;d.tabindex=h;d.id=j;d.$original=c;d.$select=a;d.value=o(a.val())||o(c.attr("value"));d.label=c.text();d.options=e;g=u('<div class="dk_container_wrapper"><div class="dk_container" id="dk_container_{{ id }}" tabindex="{{ tabindex }}"><a class="dk_toggle"><span class="dk_label">{{ label }}</span></a><div class="dk_options"><ul class="dk_options_inner"></ul></div></div></div>',
d);g.find(".dk_toggle").css({width:i+"px"});a.before(g);g=f("#dk_container_"+j).fadeIn(b.startSpeed);c=b.theme?b.theme:"default";g.addClass("dk_theme_"+c);d.theme=c;d.$dk=g;a.data("dropkick",d);g.data("dropkick",d);m[m.length]=a;g.bind("focus.dropkick",function(){g.addClass("dk_focus")}).bind("blur.dropkick",function(){g.removeClass("dk_open dk_focus")});setTimeout(function(){a.hide()},0)})};k.theme=function(b){var a=f(this).data("dropkick");a.$dk.removeClass("dk_theme_"+a.theme).addClass("dk_theme_"+
b);a.theme=b};k.reset=function(){for(var b=0,a=m.length;b<a;b++){var c=m[b].data("dropkick"),e=c.$dk,d=e.find("li").first();e.find(".dk_label").text(c.label);e.find(".dk_options_inner").animate({scrollTop:0},0);i(d,e);n(d,e,!0)}};f.fn.dropkick=function(b){if(!r){if(k[b])return k[b].apply(this,Array.prototype.slice.call(arguments,1));if(typeof b=="object"||!b)return k.init.apply(this,arguments)}};f(function(){f(".dk_toggle").live("click",function(b){var a=f(this).parents(".dk_container").first();return l(a),
"ontouchstart"in s&&(a.addClass("dk_touch"),a.find(".dk_options_inner").addClass("scrollable vertical")),b.preventDefault(),!1});f(".dk_options a").live(f.browser.msie?"mousedown":"click",function(b){var a=f(this),c=a.parents(".dk_container").first();c.data("dropkick");c.removeClass("dk_open");return n(a,c),i(a.parent(),c),b.preventDefault(),!1});f(q).bind("keydown.dk_nav",function(b){var a=f(".dk_container.dk_open"),c=f(".dk_container.dk_focus"),e=null;a.length?e=a:c.length&&!a.length&&(e=c);if(e){a=
e;e=b.keyCode;a.data("dropkick");var d=a.find(".dk_options"),c=a.hasClass("dk_open"),j=a.find(".dk_option_current"),k=d.find("li").first(),d=d.find("li").last(),h;switch(e){case p.enter:c?(n(j.find("a"),a),a.removeClass("dk_open")):l(a);b.preventDefault();break;case p.up:h=j.prev("li");c?h.length?i(h,a):i(d,a):l(a);b.preventDefault();break;case p.down:c?(h=j.next("li").first(),h.length?i(h,a):i(k,a)):l(a),b.preventDefault()}}})})})(jQuery,window,document);
...
*
**/
 ;(function(b){b.MyZebra_Form_Taxonomy=function(f,j){var j=b.extend({remove_text:"",auto_suggest:!1,ajax_url:""},j),c=function(b){return b.replace(/[\*\(\)\{\}\[\]\+\,\.]|[\f\n\r\t\v\u00A0\u2028\u2029]+/g,"")},k=f.children(".myzebra-taxonomy-addremove").eq(0),m=k.attr("name"),r=function(){var b=k.val().match(/^add\{(.*?)\}remove\{(.*?)\}$/i),c=b[1].split(","),b=b[2].split(",");if(c==""||c[0]==""||!c.length)c=[];if(b==""||b[0]==""||!b.length)b=[];return{add:c,remove:b}},q=function(b,c){k.val("add{"+
b.join(",")+"}remove{"+c.join(",")+"}")},o=function(){var c=[];s.find(".myzebra-term").each(function(){c.push(b(this).text())});return c},x=function(g,k,o){g=b.trim(g);if(g!=""){for(var m=r(),l=g.split(","),u=0;u<l.length;u++)if(g=!0,l[u]=c(l[u]),l[u]!=""){var a=b.inArray(l[u],m.remove);a>-1&&(m.remove.splice(a,1),g=!1);s.find(".myzebra-term").filter(function(){return b(this).text()==l[u]?!0:!1}).length||(!o&&g&&m.add.push(l[u]),g=b('<div class="myzebra-term-wrapper"><a class="myzebra-term-remove" href="javascript:void(0);" title="'+
j.remove_text+'"></a><span class="myzebra-term">'+l[u]+"</span></div>"),k?g.appendTo(s).hide().fadeIn("slow"):g.appendTo(s))}o||f.trigger("taxonomy.change");l=null;q(m.add,m.remove)}},t=function(g,j,k){var g=g.parent(".myzebra-term-wrapper"),o=g.children(".myzebra-term").eq(0).text(),m=r(),o=c(o),l=b.inArray(o,m.add),a=!0;l>-1&&(m.add.splice(l,1),a=!1);o!=""&&b.inArray(o,m.remove)==-1&&a&&!k&&m.remove.push(o);q(m.add,m.remove);j?g.fadeOut("slow",function(){b(this).remove();k||f.trigger("taxonomy.change")}):
(g.remove(),k||f.trigger("taxonomy.change"))},v=f.children(".myzebra-text").eq(0);f.parent("form");var s=f.find(".myzebra-terms").eq(0);v.keyup(function(c){if(13==c.which)return x(b.trim(jQuery(this).val()),!0,!1),b(this).val(""),!1}).keypress(function(b){if(13==b.which)return b.preventDefault(),!1});j.auto_suggest&&v.suggest(j.ajax_url+"?action=cred-ajax-tag-search&tax="+m,{delay:500,minchars:2,multiple:!0,multipleSep:", ",resultsClass:"myzebra-ac_results",selectClass:"myzebra-ac_over",matchClass:"myzebra-ac_match"});
f.on("click",".myzebra-terms a.myzebra-term-remove",function(){t(b(this),!0,!1)});f.children(".myzebra-add-new-term").click(function(){var c=b.trim(v.val());if(c=="")return!1;x(c,!0,!1);v.val("")});for(var g=r(),l=0;l<g.add.length;)m=b.inArray(g.add[l],g.remove),m>-1&&g.remove.splice(m,1),s.find(".myzebra-term").filter(function(){return b(this).text()==g.add[l]?!0:!1}).length>0?(g.add.splice(l,1),l=0):l++;for(l=0;l<g.remove.length;)s.find(".myzebra-term").filter(function(){return b(this).text()==
g.remove[l]?!0:!1}).length==0?(g.remove.splice(l,1),l=0):l++;q(g.add,g.remove);m=b.trim(v.val());m!=""&&x(m,!1,!1);v.val("");x(r().add.join(","),!1,!0);for(var y=r().remove,l=0;l<y.length;l++)s.find(".myzebra-term").filter(function(){return b(this).text()==y[l]?!0:!1}).each(function(){t(b(this),!1,!0)});f.data("addTerm",function(b){x(b,!0,!1)});f.data("getTerms",function(){return o()})};b.fn.MyZebra_Form_Taxonomy=function(f){return this.each(function(){b.MyZebra_Form_Taxonomy(b(this),f)})};b.MyZebra_Form_TaxonomyPopular=
function(f,j){var j=b.extend({mastercontrol:null,show_popular_text:"",hide_popular_text:""},j),c=j.mastercontrol,k=f.find(".myzebra-terms-popular").eq(0);k.find(".myzebra-term-popular").click(function(){c.data("addTerm")(jQuery(this).text())});f.find(".myzebra-show-hide-popular").click(function(){k.is(":visible")?(k.hide(),b(this).text(j.show_popular_text)):(k.show(),b(this).text(j.hide_popular_text))});k.hide()};b.fn.MyZebra_Form_TaxonomyPopular=function(f){return this.each(function(){b.MyZebra_Form_TaxonomyPopular(b(this),
f)})};b.MyZebra_Form_TaxonomyHierarchical=function(f){var j=b(f),f=j.find(".myzebra-taxonomy-hierarchical"),c,k;c=f.find(".myzebra-taxonomy-hierarchical-select-container");k=f.find(".myzebra-taxonomy-hierarchical-checkbox-container");var m=function(){var f=[];c.length?c.find("select option:selected").each(function(){f.push(b(this).text())}):k.length&&k.find('input[type="checkbox"]:checked').each(function(){f.push(b(this).siblings(".myzebra-checkbox-label-span").text())});return f};j.data("getTerms",
function(){return m()});if(c.length)c.find("select").change(function(b){b.stopPropagation();j.trigger("taxonomy.change")});else if(k.length)k.on("change",'input[type="checkbox"]',function(b){b.stopPropagation();j.trigger("taxonomy.change")})};b.fn.MyZebra_Form_TaxonomyHierarchical=function(f){return this.each(function(){b.MyZebra_Form_TaxonomyHierarchical(b(this),f)})};b.MyZebra_Form_TaxonomyHierarchicalAddNew=function(f,j){var c=j.mastercontrol.find(".myzebra-taxonomy-hierarchical"),k=j.mastercontrol.attr("id"),
m=c.find("#"+k+"_hierarchy"),k=c.find("#"+k+"_chosen").val().split(",")||[],r=f.find("select"),q,o,x=f.find(".myzebra-add-text"),t=f.find(".myzebra-add-new-term");f.find(".myzebra-add-new-hierarchical-terms").hide();f.find(".myzebra-add-new-hierarchical").click(function(){b(this).hasClass("myzebra-add-new-hierarchical-open")?(b(this).removeClass("myzebra-add-new-hierarchical-open"),f.find(".myzebra-add-new-hierarchical-terms").hide()):(b(this).addClass("myzebra-add-new-hierarchical-open"),f.find(".myzebra-add-new-hierarchical-terms").show(),
q=c.find(".myzebra-taxonomy-hierarchical-select-container"),o=c.find(".myzebra-taxonomy-hierarchical-checkbox-container"),r.empty(),r.append('<option value="-1">'+j.parent_text+"</option>"),q.length?q.find("select option").each(function(){r.append(jQuery(this).clone())}):o.length&&o.find('input[type="checkbox"]').each(function(){r.append('<option value="'+jQuery(this).val()+'">'+jQuery(this).closest(".myzebra-taxonomy-hierarchical-checkbox").find(".myzebra-checkbox-label-span").eq(0).text()+"</option>")}))});
var v=function(){var g;g=b.trim(x.val()).replace(/[\*\(\)\{\}\[\]\+\,\.]|[\f\n\r\t\v\u00A0\u2028\u2029]+/g,"");if(g=="")return!1;var k=r.val();if(q.length)q.find("select").prepend('<option value="'+g+'" selected="selected">'+g+"</option>");else if(o.length){var l=c.find(".myzebra-taxonomy-hierarchical-name-parameter").text(),l=b("<div style='position:relative;line-height:0.9em;margin:2px 0;margin-left:15px;' class='myzebra-taxonomy-hierarchical-checkbox'><label class='myzebra-style-label'><input type='checkbox' class='myzebra-control myzebra-checkbox' name='"+
l+"' value='"+g+"' checked='checked' /><span class='myzebra-checkbox-replace'></span><span class='myzebra-checkbox-label-span'  style='position:relative;font-size:12px;display:inline-block;margin:0;padding:0;margin-left:15px;'>"+g+"</span></label></div>");o.prepend(l)}x.val("");m.val(m.val()+"{"+k+","+g+"}");f.find(".myzebra-add-new-hierarchical").removeClass("myzebra-add-new-hierarchical-open");f.find(".myzebra-add-new-hierarchical-terms").hide();j.mastercontrol.trigger("taxonomy.change");return!1};
t.click(v);x.keyup(function(b){if(13==b.which)return v(),!1}).keypress(function(b){if(13==b.which)return b.preventDefault(),!1});if(m.val()!=""){q=c.find(".myzebra-taxonomy-hierarchical-select-container");o=c.find(".myzebra-taxonomy-hierarchical-checkbox-container");for(var t=m.val().match(/\{[^\{\},]+,[^\{\},]+\}/g),s=0;s<t.length;s++){var g=t[s].replace(/\{|\}/g,"").split(",");if(q.length)b.inArray(g[1],k)!=-1?q.find("select").prepend('<option value="'+g[1]+'" selected="selected">'+g[1]+"</option>"):
q.find("select").prepend('<option value="'+g[1]+'">'+g[1]+"</option>");else if(o.length){var l=c.find(".myzebra-taxonomy-hierarchical-name-parameter").text(),g=b.inArray(g[1],k)!=-1?b("<div style='position:relative;line-height:0.9em;margin:2px 0;margin-left:15px;' class='myzebra-taxonomy-hierarchical-checkbox'><input type='checkbox' class='myzebra-control myzebra-checkbox' name='"+l+"' value='"+g[1]+"' checked='checked' /><span class='myzebra-checkbox-label-span'  style='position:relative;font-size:12px;display:inline-block;margin:0;padding:0;margin-left:15px;'>"+
g[1]+"</span></div>"):b("<div style='position:relative;line-height:0.9em;margin:2px 0;margin-left:15px;' class='myzebra-taxonomy-hierarchical-checkbox'><input type='checkbox' class='myzebra-control myzebra-checkbox' name='"+l+"' value='"+g[1]+"' /><span class='myzebra-checkbox-label-span'  style='position:relative;font-size:12px;display:inline-block;margin:0;padding:0;margin-left:15px;'>"+g[1]+"</span></div>");o.prepend(g)}}}};b.fn.MyZebra_Form_TaxonomyHierarchicalAddNew=function(f){return this.each(function(){b.MyZebra_Form_TaxonomyHierarchicalAddNew(b(this),
f)})};b.fn.fadeToggle=function(f,j,c){j=j||"linear";return this.each(function(){b(this).animate({opacity:"toggle"},f,j,function(){jQuery.browser.msie&&this.style.removeAttribute("filter");jQuery.isFunction(c)&&c()})})};b.fn.slideFadeToggle=function(f,j,c){j=j||"linear";return this.each(function(){b(this).animate({opacity:"toggle",height:"toggle"},f,j,function(){jQuery.browser.msie&&this.style.removeAttribute("filter");jQuery.isFunction(c)&&c()})})};b.fn.slideFadeDown=function(f,j,c){j=j||"linear";
return this.each(function(){b(this).animate({opacity:"show",height:"show"},f,j,function(){jQuery.browser.msie&&this.style.removeAttribute("filter");jQuery.isFunction(c)&&c()})})};b.fn.slideFadeUp=function(f,j,c){j=j||"linear";return this.each(function(){b(this).animate({opacity:"hide",height:"hide"},f,j,function(){jQuery.browser.msie&&this.style.removeAttribute("filter");jQuery.isFunction(c)&&c()})})};(function(b){b.cleditor.buttons.insertMedia={name:"insertMedia",css:{background:"url("+myzebra.insertMediaIconURL+
") no-repeat center center"},title:"Insert Media",command:"inserthtml",popupName:"insertMedia",popupClass:"cleditorPopup",popupContent:"",buttonClick:function(j,c){var k=c.editor;k.hidePopups();c.popup&&b(c.popup).remove();var m=k.$area.closest("form").find('input[name="'+myzebra.PREFIX+'post_id"]');if(m.length)return m=m.val(),window.send_to_editor=function(b){k.execCommand(c.command,b,null,c.button);tb_remove();k.focus()},tb_show("Insert Media",myzebra.insertMediaPopupURL+"?post_id="+m+"&amp;type=image&amp;TB_iframe=true"),
!1}};b.cleditor.buttons.source.buttonClick=function(b,c){var f=c.editor;if(f.disabled)return!0;var m=f.$area.closest(".myzebra-rich-editor-container").find(".myzebra-rich-editor-resize");f.sourceMode()?m.show():m.hide();return!0}})(b);b.MyZebra_Form=function(f,j){var c=this,k=j.conditionals!==!1,m=j.conditionals,r={scroll_to_error:!0,tips_position:"left",close_tips:!0,validate_on_the_fly:!1,validate_all:!1,assets_path:null,myzebra_datepicker:{}},q={};c.settings={};c.editor_resize=null;var o={},x=
{},t={},v=[],s=!1;c.remove_field_action=function(a){var a=b(a),a=a.closest(".myzebra-repeatable-field"),i=a.find(".myzebra-control").eq(0).attr("id");b("#MyZebra_Form_error_message_"+i).remove();b("#"+i+"_spinner").remove();delete o[i];delete x[i];delete t[i];delete c.settings.error_messages[i];delete q[i];a.fadeOut("slow",function(){b(this).remove()})};var g=b(f),l=f;c.filter_input=function(a,b,e){var d,c="";if(window.event)d=window.event.keyCode,b=window.event;else if(b)d=b.which;else return!0;
switch(a){case "alphabet":c="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";break;case "digits":case "number":case "float":c="0123456789";break;case "alphanumeric":c="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";break;default:return!0}e&&(c+=e);b=b.srcElement?b.srcElement:b.target||b.currentTarget;if(d==null||d==0||d==8||d==9||d==13||d==27)return!0;d=String.fromCharCode(d);if(c.indexOf(d)>-1)return!0;if(a=="number"&&d=="-"&&A(b)==0)return!0;if(a=="float"&&(d=="-"&&A(b)==
0||d=="."&&A(b)!=0&&b.value.match(/\./)==null))return!0;return!1};c.element_add=function(a){var i={id:a.attr("id"),type:u(a)},e=a.closest(".row");a.unbind("click focus blur change keypress");a.hasClass("myzebra-date")&&!a.is("[readonly]")&&a.MyZebra_DatePicker(q[i.id]);e.length&&a.bind({focus:function(){e.addClass("myzebra-highlight")},blur:function(){e.removeClass("myzebra-highlight")}});if(a.hasClass("myzebra-inner-label")&&(i.type=="text"||i.type=="password"||i.type=="textarea")){var d=a.position(),
h=i.type=="text"||i.type=="password"?jQuery("<input>").attr({type:"text","class":"MyZebra_Form_Placeholder",autocomplete:"off",value:a.attr("title")}):jQuery("<textarea>").attr({"class":"MyZebra_Form_Placeholder",autocomplete:"off"}).html(a.attr("title"));h.css({display:"none",fontFamily:a.css("fontFamily"),fontSize:a.css("fontSize"),fontStyle:a.css("fontStyle"),fontWeight:a.css("fontWeight"),left:d.left,top:d.top,width:a.width()+(parseInt(a.css("borderLeftWidth"),10)||0)+(parseInt(a.css("borderRightWidth"),
10)||0),height:a.height()+(parseInt(a.css("borderTopWidth"),10)||0)+(parseInt(a.css("borderBottomWidth"),10)||0),paddingTop:parseInt(a.css("paddingTop"),10)||0,paddingRight:parseInt(a.css("paddingRight"),10)||0,paddingBottom:parseInt(a.css("paddingBottom"),10)||0,paddingLeft:parseInt(a.css("paddingLeft"),10)||0}).insertAfter(a);a.removeAttr("title");h.bind("focus",function(){a.focus()});a.bind({focus:function(){h.css("display","none")},blur:function(){b(this).val()==""&&h.css("display","block")}});
a.data("MyZebra_Form_Placeholder",h);v.push(a)}else a.hasClass("myzebra-other")&&i.type=="select-one"&&(B(a),a.change(function(){B(a)}));void 0!=c.settings.error_messages&&void 0!=typeof c.settings.error_messages[i.id]&&c.register(a,!1);if((i.type=="text"||i.type=="textarea"||i.type=="password")&&a.attr("maxlength"))if(a.data("maxlength",a.attr("maxlength")),a.bind("keyup",function(){var a=b(this),d=a.data("maxlength"),c=D(a);a.attr("maxlength",d-c);a.hasClass("myzebra-show-character-counter")&&(a=
d-c-a.val().length,g.html(a<0?"<span>"+a+"</span>":a))}),a.hasClass("myzebra-show-character-counter")){var d=a.position(),g=jQuery("<div>",{"class":"MyZebra_Character_Counter",css:{visibility:"hidden"}}).html(a.data("maxlength")).insertAfter(a);width=g.outerWidth();height=g.outerHeight();g.css({top:d.top+a.outerHeight()-height/1.5,left:d.left+a.outerWidth()-width/1.5,width:g.width(),visibility:"visible"});a.trigger("keyup")}};c.hideShowGroup=function(a,b,c){if(b)switch(a.parents(".myzebra-conditional-group-hidden").length||
a.find(".myzebra-control").not(".myzebra-file-disabled").removeAttr("disabled"),a.removeClass("myzebra-conditional-group-hidden").addClass("myzebra-conditional-group-visible"),c){case "fade-slide":setTimeout(function(){a.stop(!0).slideFadeDown("slow","quintEaseOut")},50);break;case "slide":setTimeout(function(){a.stop(!0).slideDown("slow","quintEaseOut")},50);break;case "fade":setTimeout(function(){a.stop(!0).fadeIn("slow")},50)}else switch(a.find(".myzebra-control").attr("disabled","disabled"),a.removeClass("myzebra-conditional-group-visible").addClass("myzebra-conditional-group-hidden"),
c){case "fade-slide":setTimeout(function(){a.stop(!0).slideFadeUp("slow","quintEaseIn")},50);break;case "slide":setTimeout(function(){a.stop(!0).slideUp("slow","quintEaseIn")},50);break;case "fade":setTimeout(function(){a.stop(!0).fadeOut("slow")},50)}};c.getFieldVal=function(a,c){var e=null,d;a.hasClass("myzebra-taxonomy")&&a.data("getTerms")?e=a.data("getTerms")():a.hasClass("myzebra-checkbox")?(e=[],a.filter(function(){if(b(this).is(":checked"))return d=b(this).val(),c&&c[d]&&(d=c[d]),e.push(d),
!0;return!1})):a.hasClass("myzebra-select")?(e=[],a.find("option").filter(function(){if(b(this).is(":selected"))return d=b(this).val(),c&&c[d]&&(d=c[d]),e.push(d),!0;return!1})):a.hasClass("myzebra-radio")?a.filter(function(){if(b(this).is(":checked"))return d=b(this).val(),c&&c[d]&&(d=c[d]),e=d,!0;return!1}):e=a.eq(0).val();return e};c.runConditional=function(a){var b,e,d,h,f;e=[];if(a.map.length)for(d=0;d<a.map.length;d++){if(!a.map[d].formfield)a.map[d].formfield=g.find(".myzebra-prime-name-"+
a.map[d].field_name);h=a.map[d].formfield;f=c.getFieldVal(h,a.map[d].values_map);h.hasClass("myzebra-date")?e.push({name:a.map[d].variable,val:f,withType:"date",format:c.settings.myzebra_datepicker[a.map[d].field].format}):typeof f=="object"&&(f instanceof Array||Object.prototype.toString.call(f)=="[object Array]")?e.push({name:a.map[d].variable,val:f,withType:"array"}):e.push({name:a.map[d].variable,val:f,withType:"string"})}if(a.RESULT)b=a.RESULT;else try{b=a.parser(e)}catch(w){typeof w=="string"||
w instanceof String?c.log("Parser Error: "+w):c.log("Parser Error: "+w.message),showeGroup=!1}if(!a.map.length&&!a.RESULT)a.RESULT=b;if(!a.$group)a.$group=g.find("#"+a.group);(b&&!a.$group.hasClass("myzebra-conditional-group-visible")||!b&&!a.$group.hasClass("myzebra-conditional-group-hidden"))&&c.hideShowGroup(a.$group,b,a.mode);return a};c.conditionalControl=function(){var a=b(this).data("_conditionalControl");if(a){var i,e,d,h,f,w,j;j=!1;for(var n=0;n<a.length;n++){e=a[n];d=[];if(e.map.length)for(h=
0;h<e.map.length;h++){if(!e.map[h].formfield)e.map[h].formfield=g.find(".myzebra-prime-name-"+e.map[h].field_name),j=!0;f=e.map[h].formfield;w=c.getFieldVal(f,e.map[h].values_map);f.hasClass("myzebra-date")?d.push({name:e.map[h].variable,val:w,withType:"date",format:c.settings.myzebra_datepicker[e.map[h].field].format}):typeof w=="object"&&(w instanceof Array||Object.prototype.toString.call(w)=="[object Array]")?d.push({name:e.map[h].variable,val:w,withType:"array"}):d.push({name:e.map[h].variable,
val:w,withType:"string"})}if(e.RESULT)i=e.RESULT;else try{i=e.parser(d)}catch(o){typeof o=="string"||o instanceof String?c.log("Parser Error: "+o):c.log("Parser Error: "+o.message),showeGroup=!1}if(!e.map.length&&!e.RESULT)e.RESULT=i,j=!0;if(!e.$group)e.$group=g.find("#"+e.group),j=!0;(i&&!e.$group.hasClass("myzebra-conditional-group-visible")||!i&&!e.$group.hasClass("myzebra-conditional-group-hidden"))&&c.hideShowGroup(e.$group,i,e.mode);a[n]=e}j&&b(this).data("_conditionalControl",a)}};c.log=function(a){window.console&&
window.console.log&&window.console.log(a)};c.initConditionals=function(){if(k&&MyZebraParser){var a;myzebra.parser_info&&MyZebraParser.setParams(myzebra.parser_info);try{a=(new MyZebraParser.Expression).dateLocales(myzebra.days,myzebra.months)}catch(b){typeof b=="string"||b instanceof String?c.log("Init Parser Error: "+b):c.log("Init Parser Error: "+b.message);return}for(var e={},d=0;d<m.groups.length;d++){var h=m.groups[d];h.parser=a.preCompile(h.condition);for(var f=0;f<h.affected_fields_names.length;f++){var w=
h.affected_fields_names[f],j=g.find(".myzebra-prime-name-"+w);if(j.length){var n=j.data("_conditionalControl");n?n.push(h):n=[h];j.data("_conditionalControl",n);e[w]||(j.hasClass("myzebra-taxonomy")?j.bind("taxonomy.change",c.conditionalControl):j.bind("change",c.conditionalControl),e[w]=!0)}}}setTimeout(function(){for(var a=0;a<m.groups.length;a++)c.runConditional(m.groups[a])},100)}};c.init=function(){c.settings=b.extend({},r,j);q=c.settings.myzebra_datepicker;g.find("option.myzebra-dummy").remove();
b("div.myzebra-form-error",g).each(function(){b("div.myzebra-close a",b(this)).bind("click",function(a){a.preventDefault();b(this).closest("div.myzebra-form-error").animate({height:0,opacity:0},250,function(){b(this).remove()})})});b("div.myzebra-form-message",g).each(function(){b("div.myzebra-close a",b(this)).bind("click",function(a){a.preventDefault();b(this).closest("div.myzebra-form-message").animate({height:0,opacity:0},250,function(){b(this).remove()})})});b(".myzebra-control",g).each(function(){c.element_add(b(this))});
g.on("mousedown",".myzebra-rich-editor-resize",function(a){var e=b(this).closest(".myzebra-rich-editor-container"),d=e.find(".myzebra-wysiwyg");if(d.length){var h=d.data("editor");if(h&&!h.disabled)h.updateTextArea(),c.editor_resize={element:d,container:e,pX:a.pageX,pY:a.pageY,h:e.height(),w:e.width()}}return!1});b(document).bind("mouseup",function(){if(c.editor_resize&&c.editor_resize.element.length){var a=c.editor_resize.element.data("editor");a.updateFrame();a.refresh();c.editor_resize=null}c.editor_resize=
null});b(document).bind("mousemove",function(a){if(c.editor_resize&&c.editor_resize.container.length){var b=c.editor_resize.container;b.height(c.editor_resize.h-c.editor_resize.pY+a.pageY);b.width(c.editor_resize.w-c.editor_resize.pX+a.pageX)}});g.find(".myzebra-add-new-field").bind("click",function(){var a=b(this).parent("div"),e=parseInt(a.attr("class").replace(/myzebra\-count_(\d+)/,"$1")),d=a.find(".myzebra-repeatable-field");if(a.find(".myzebra-skype-container").length){var h=d.eq(0).find(".myzebra-skype-container").eq(0),
g=h.attr("id").replace(/_repeat_(\d+)/,""),f=h.find(".myzebra-control").eq(0),j=f.attr("id").replace(/_repeat_(\d+)/,""),h=h.find(".myzebra-hidden").eq(0),n=f.attr("name"),n=f.attr("name").replace(/\[[^\[\]]*\]$/,"["+e+"]"),o=h.attr("name").replace(/\[[^\[\]]*\]$/,"["+e+"]"),f=g+"_repeat_"+e,d=d.eq(0).clone(!0);d.find(".myzebra-validation-error").length>0&&(d.find(".myzebra-field-error-messages").remove(),d.find(".myzebra-validation-error").children(":eq(0)").unwrap());h=d.find(".myzebra-control").eq(0);
clone_element_hidden=d.find(".myzebra-hidden").eq(0);h.removeAttr("disabled");h.removeClass("myzebra-file-disabled");h.removeData();h.attr("id",h.attr("id").replace(g,f));clone_element_hidden.attr("id",clone_element_hidden.attr("id").replace(g,f));clone_element_link=d.find(".myzebra-edit-skype").eq(0);clone_element_link.attr("href",clone_element_link.attr("href").replace(g,f));h.attr("name",n);clone_element_hidden.attr("name",o);d.find(".myzebra-skype-container").attr("id",d.find(".myzebra-skype-container").attr("id").replace(g,
f));d.find(".myzebra-skype-preview-image-container").attr("id",d.find(".myzebra-skype-preview-image-container").attr("id").replace(g,f));c.settings.error_messages[j]&&(c.settings.error_messages[h.attr("id")]=b.extend({},c.settings.error_messages[j]));a.append(d);c.element_add(h);g=b("<a href='javascript:;' class='myzebra-remove-field'>"+myzebra.remove_repeatable_field+"</a>");g.insertAfter(h)}else h=d.eq(0).find(".myzebra-control").eq(0),g=h.attr("id").replace(/_repeat_(\d+)/,""),n=h.attr("name").replace(/\[[^\[\]]*\]$/,
"["+e+"]"),f=g+"_repeat_"+e,j={id:h.attr("id"),type:u(h)},d=d.eq(0).clone(!0),d.children(".myzebra-validation-error").length>0&&(d.find(".myzebra-field-error-messages").remove(),d.children(".myzebra-validation-error").children(":eq(0)").unwrap()),h.hasClass("myzebra-file")&&d.find('input[type="text"]').parent("div").remove(),h.hasClass("myzebra-date")&&d.find(".MyZebra_DatePicker_Icon").remove(),h=d.find(".myzebra-control").eq(0),h.removeAttr("disabled"),h.removeData(),h.attr("id",f),h.attr("name",
n),h.val(""),h.attr("value",""),c.settings.error_messages[g]&&(c.settings.error_messages[f]=b.extend({},c.settings.error_messages[g])),j.type=="file"&&c.settings.error_messages[f].file_value&&delete c.settings.error_messages[f].file_value,h.hasClass("myzebra-date")&&(q[f]=b.extend({},q[g])),a.append(d),c.element_add(h),h=d.find(".myzebra-control").eq(0),h.css("visibility","visible").show(),g=b("<a href='javascript:;' class='myzebra-remove-field'>"+myzebra.remove_repeatable_field+"</a>"),h.hasClass("myzebra-date")?
g.insertAfter(h.parent(".myzebra-date-container")):g.insertAfter(h);g.unbind("click").bind("click",function(){c.remove_field_action(this)});a.attr("class","myzebra-count_"+ ++e);d.hide().fadeIn("slow")});g.find(".myzebra-remove-field").each(function(){var a=b(this),e=a.closest(".myzebra-repeatable-field"),d=e.find(".myzebra-control").eq(0),e=e.find(".myzebra-file-text").eq(0);e.length>0?a.remove().insertAfter(e):a.remove().insertAfter(d);a.unbind("click").bind("click",function(){c.remove_field_action(this)})});
b("body").on("submit","#"+g.attr("id"),function(a){s==!1&&void 0!=c.settings.error_messages&&(c.validate()?void 0==a&&g.submit():(void 0!=a&&a.preventDefault(),c.show_errors()))});v.length>0&&setInterval(y,100);var a=g.find(".myzebra-validation-error");a.length&&b("html").scrollTop(a.eq(0).offset().top-80);c.initConditionals()};c.attach_tip=function(a,b){var e=a.attr("id");o[e].message=b;c.show_errors(a)};c.clear_errors=function(){g.find(".MyZebra_Form_error_iFrameShim").remove();g.find(".MyZebra_Form_error_message").remove();
t=[]};c.handle_file=function(a,c,e,d){typeof d=="undefined"&&(d=!1);var h=jQuery('<div class="myzebra-file-text"/>').insertAfter(a),c=jQuery("<input>",{id:a.attr("id")+"_text","class":a.attr("class"),style:"position:relative;",type:"text",name:a.attr("name"),value:e});h.append(c);d&&(a.attr("disabled","disabled"),a.hasClass("myzebra-file-disabled")||a.addClass("myzebra-file-disabled"));a.hide();d=jQuery("<a>",{href:"javascript:void(0)","class":"myzebra-file-text-close",title:myzebra.cancel_upload_text}).unbind("click").bind("click",
function(c){c.preventDefault();h.remove();a.val("");a.data("file_info")&&a.removeData("file_info");b("#"+a.attr("id")+"_spinner").remove();a.css("visibility","visible").show();a.removeAttr("disabled");f.disabled=!1;!a.hasClass("myzebra-file-disabled")||a.removeClass("myzebra-file-disabled")});h.append(d)};c.end_file_upload=function(a,i){var e=b("#"+a);e.length&&(c.clear_errors(),g.removeAttr("target"),b("#"+a+"_iframe").remove(),b("#"+a+"_spinner").remove(),void 0!=o[a]&&(void 0!=i&&"object"==typeof i&&
void 0!=i[0]&&void 0!=i[1]&&void 0!=i[2]&&void 0!=i[3]&&void 0!=i[4]&&void 0!=i[5]&&e.data("file_info",i),!0!==c.validate_control(e)?(e.val(""),e.css("visibility","visible"),c.show_errors(e)):c.handle_file(e,i,i[0],!1)))};c.hide_error=function(a,i){if(a){var e=b("#"+a);if(e.length){var d={id:e.attr("id"),name:e.attr("name"),type:u(e)};d.name=d.name.replace(/\[[^\[\]]*\]$/,"")}if(void 0==i&&c.settings.validate_on_the_fly&&!0!==c.validate_control(e))c.show_errors(e,!1);else{var h=a;if(e.length&&(d.type==
"radio"||d.type=="checkbox"))h=d.name;var g=b("#MyZebra_Form_error_message_"+h);g.length>0&&g.animate({opacity:0},250,function(){var b=g.data("shim");void 0!=b&&b.remove();g.remove();delete t[a]})}}};c.register=function(a,i){var e={id:a.attr("id"),name:a.attr("name"),type:u(a)},d=e.name;e.name=e.name.replace(/\[[^\[\]]*\]$/,"");a.is("select")&&!a.closest(".myzebra-keep-original").length&&!a.closest(".cred-keep-original").length&&a.dropkick();switch(e.type){case "radio":case "checkbox":a.bind({click:function(){c.hide_error(e.id)},
blur:function(){c.hide_error(e.id)}});void 0==x[e.id]&&(x[e.id]=g.find('input[name="'+d+'"]'));break;case "file":var h=a.clone(!0),d=c.settings.error_messages[a.attr("id")].file_value?c.settings.error_messages[a.attr("id")].file_value:"";h.attr("value","");a.replaceWith(h);h.bind({keypress:function(a){a.preventDefault();h.attr("value","")},change:function(){if(void 0!=o[e.id].rules.upload){c.hide_error(e.id);h.data("file_info")&&h.removeData("file_info");var d=jQuery('<iframe id="'+e.id+'_iframe" name="'+
e.id+'_iframe">',{src:"javascript:void(0)",scrolling:"no",marginwidth:0,marginheight:0,width:0,height:0,frameborder:0,allowTransparency:"true"});b("body").append(d);var d=g.attr("action"),i=a.attr("name").match(/\.*\[(\d+?)\]$/),i=i!=null?i[1]:0;g.attr("action",decodeURIComponent(c.settings.assets_path)+"process.php?form="+g.attr("id")+"&control="+e.id+"&index="+(a.closest(".myzebra-repeatable-field").length>0?i:0)+"&controlname="+e.name+"&path="+encodeURIComponent(decodeURIComponent(o[e.id].rules.upload[0]))+
"&url="+encodeURIComponent(decodeURIComponent(o[e.id].rules.upload[1]))+"&nocache="+(new Date).getTime());g.attr("target",e.id+"_iframe");a.css("visibility","hidden");i=a.offset();i=jQuery("<div>",{id:e.id+"_spinner","class":"MyZebra_Form_spinner",css:{left:i.left,top:i.top}});b("body").append(i);s=!0;g.trigger("submit");g.attr("action",d);s=!1}},blur:function(){c.hide_error(e.id)}});a=h;d!=""&&c.handle_file(a,null,d,!0);break;case "select-one":case "select-multiple":a.unbind("change blur").bind({change:function(){c.hide_error(e.id)},
blur:function(){c.hide_error(e.id)}});break;default:a.unbind("blur").blur(function(){c.hide_error(e.id)})}d=a.attr("class").match(/validate\[(.+)\]/);if(null!=d){for(var f=/([^\,]*\(.*?\)|[^\,]+)/g,j={};matches=f.exec(d[1]);){var m=matches[1].match(/^([^\(]+)/),n=matches[1].match(/\((.*?)\)/);n?(n=n[1].split(","),b.each(n,function(a){n[a]=n[a].replace(/lsqb\;/g,"[");n[a]=n[a].replace(/rsqb\;/g,"]");n[a]=n[a].replace(/comma\;/g,",");n[a]=n[a].replace(/lsb\;/g,"(");n[a]=n[a].replace(/rsb\;/g,")")})):
n=null;j[m[1]]=n}if(void 0==i){var l=g.find(".myzebra-control");b.each(l,function(c,d){if(d==a.get(0)){b(d);for(var i=null,h=c-1;i==null&&void 0!=l[h];)i=b(l[h]).attr("id"),h--;if(o[i]){h={};for(c in o)h[c]=o[c],i==c&&(h[e.id]={element:a,rules:j});o=h}else i={},i[e.id]={element:a,rules:j},b.extend(o,i)}})}else if(void 0!=i&&b("#"+i).length){b("#"+i).attr("id");d={};for(index in o)d[index]=o[index],previous_element_id==index&&(d[e.id]={element:a,rules:j});o=d}else void 0!=i&&i===!1&&(o[e.id]={element:a,
rules:j})}a.hasClass("myzebra-wysiwyg")&&(d="bold italic underline strikethrough | bullets numbering style image link | cut copy paste pastetext | undo redo | source",a.hasClass("myzebra-has-media-button")&&(d="insertMedia "+d),""==b.trim(a.val())&&a.val("<p>&nbsp;</p>"),d=a.cleditor({width:"100%",height:"100%",controls:d,styles:[["Paragraph","<p>"],["Header 1","<h1>"],["Header 2","<h2>"],["Header 3","<h3>"],["Header 4","<h4>"],["Header 5","<h5>"],["Header 6","<h6>"]],useCSS:!1,docType:'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">',
docCSSFile:"",bodyStyle:"position:relative;"})[0],a.is("[readonly]")||a.hasClass("myzebra-disabled")?d.disable(!0):d.disable(!1),b.browser.msie||d.doc.execCommand("insertBrOnReturn",!1,!1),a.data("editor",d))};c.show_errors=function(a,i){void 0!=i&&i===!1||c.clear_errors();var e=0;for(index in o){var d=o[index],h=d.element,g={id:h.attr("id"),name:h.attr("name"),type:u(h)};g.name=g.name.replace(/\[[^\[\]]*\]$/,"");var f=g.type=="radio"||g.type=="checkbox"?g.name:g.id;if(!(void 0!=a&&a.get(0)!=h.get(0))&&
void 0!=d.message&&void 0==t[f]){h.hasClass("myzebra-style-hidden")&&(h=h.parent().children("a").eq(0));h.hasClass("myzebra-select")&&b("#dk_container_"+h.attr("id")).length&&(h=b("#dk_container_"+h.attr("id")));h.css("display")!="none"&&!(void 0!=i&&i===!1)&&!(c.settings.validate_all&&e>0)&&h.focus();var j=b.extend(h.offset()),j=b.extend(j,{right:Math.floor(j.left+h.width())}),n=jQuery("<div/>",{"class":"MyZebra_Form_error_message",id:"MyZebra_Form_error_message_"+f,css:{opacity:0}}),d=jQuery("<div/>",
{"class":"myzebra-message"+(!c.settings.close_tips?" myzebra-noclose":""),css:{_width:"auto"}}).html(d.message).appendTo(n);c.settings.close_tips&&jQuery("<a/>",{href:"javascript:void(0)","class":"myzebra-close"+(b.browser.msie&&b.browser.version.match(/^6/)?"-ie6":"")}).html("x").appendTo(d).unbind("click focus").bind({click:function(a){a.preventDefault();c.hide_error(b(this).closest("div.MyZebra_Form_error_message").attr("id").replace(/^MyZebra\_Form\_error\_message\_/,""),!0)},focus:function(){b(this).blur()}});
var m=jQuery("<div/>",{"class":"myzebra-arrow"}).appendTo(n);b("body").append(n);var d={x:n.outerWidth(),y:n.outerHeight()},l={x:m.outerWidth(),y:m.outerHeight()},p=(c.settings.tips_position=="left"?j.left:j.right)-d.x/2;m.css("left",d.x/2-l.x/2-1);if(g.type=="radio"||g.type=="checkbox")p=j.right-d.x/2-h.outerWidth()/2+1;p<0&&(p=2);n.css("left",p);d={x:n.outerWidth(),y:n.outerHeight()};h=j.top-d.y+l.y/2-1;h<0&&(h=2);n.css({left:p+"px",top:h+"px",height:d.y-l.y/2+"px"});t[f]=n;E(n);n.animate({opacity:0.9},
250);++e==1&&c.settings.scroll_to_error&&!(void 0!=i&&i===!1)&&b("html, body").animate({scrollTop:Math.max(parseInt(n.css("top"),10)+parseInt(n.css("height"),10)/2-b(window).height()/2,0)},0);if(!c.settings.validate_all)break}}};c.submit=function(){void 0!=c.settings.error_messages?g.trigger("submit"):l.submit()};c.validate_control=function(a){var i={id:a.attr("id"),type:u(a)},e=!0,d=o[i.id];if(void 0!=d&&(a.hasClass("myzebra-style-hidden")||a.hasClass("myzebra-select")||a.attr("disabled")!="disabled"&&
a.css("display")!="none"&&a.css("visibility")!="hidden"||a.data("file_info"))){var h=null,g=null;delete d.message;for(var f in d.rules){if(!e)break;switch(f){case "alphabet":switch(i.type){case "password":case "text":case "textarea":var j=RegExp("^[a-z"+z(d.rules[f][0]).replace(/\s/,"\\s")+"]+$","ig");b.trim(a.val())!=""&&!j.test(a.val())&&(e=!1)}break;case "alphanumeric":switch(i.type){case "password":case "text":case "textarea":j=RegExp("^[a-z0-9"+z(d.rules[f][0]).replace(/\s/,"\\s")+"]+$","ig"),
b.trim(a.val())!=""&&!j.test(a.val())&&(e=!1)}break;case "compare":switch(i.type){case "password":case "text":case "textarea":if(!b("#"+d.rules[f][0])||a.val()!=b("#"+d.rules[f][0]).val())e=!1}break;case "custom":var n=!1;b.each(d.rules[f],function(c,d){if(!n){var d=b.map(d.split(","),function(a){return a.replace(/mark\;/g,",")}),d=b.merge(b.merge([d.shift()],[a.val()]),d),f=typeof d[0]=="function"?d[0]:typeof window[d[0]]=="function"?window[d[0]]:!1;if(f!==!1)e=f.apply(f,d.slice(1));else throw e=
!1,Error('Function "'+d[0]+"\" doesn't exist!");e||(g=d[0],n=!0)}});break;case "date":switch(i.type){case "text":if(b.trim(a.val())!=""){for(var j=!1,m=a.data("MyZebra_DatePicker").settings.format.replace(/\s/g,""),l=["d","D","j","l","N","S","w","F","m","M","n","Y","y","G","H","g","h","a","A","i","s","U"],p=[],k=[],m=z(m),q=0;q<l.length;q++)(position=m.indexOf(l[q]))>-1&&p.push({character:l[q],position:position});p.sort(function(a,b){return a.position-b.position});b.each(p,function(a,b){switch(b.character){case "d":k.push("0[1-9]|[12][0-9]|3[01]");
break;case "D":k.push("[a-z]{3}");break;case "j":k.push("[1-9]|[12][0-9]|3[01]");break;case "l":k.push("[a-z]+");break;case "N":k.push("[1-7]");break;case "S":k.push("st|nd|rd|th");break;case "w":k.push("[0-6]");break;case "F":k.push("[a-z]+");break;case "m":k.push("0[1-9]|1[012]+");break;case "M":k.push("[a-z]{3}");break;case "n":k.push("[1-9]|1[012]");break;case "Y":k.push("[0-9]{4}");break;case "y":k.push("[0-9]{2}");break;case "G":case "H":case "g":case "h":k.push("[0-9]{1,2}");break;case "a":case "A":k.push("(am|pm)");
break;case "i":case "s":k.push("[012345][0-9]")}});if(k.length>0&&(p.reverse(),b.each(p,function(a,b){m=m.replace(b.character,"("+k[k.length-a-1]+")")}),k=RegExp("^"+m+"$","ig"),segments=k.exec(a.val().replace(/\s/g,"")))){var s=null,r=null,t=null,v=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],y=["January","February","March","April","May","June","July","August","September","October","November","December"],A=null,C=!0;p.reverse();b.each(p,function(c,d){if(!C)return!0;switch(d.character){case "m":case "n":r=
parseInt(segments[c+1],10);break;case "d":case "j":s=parseInt(segments[c+1],10);break;case "D":case "l":case "F":case "M":A=d.character=="D"||d.character=="l"?a.data("MyZebra_DatePicker").settings.days:a.data("MyZebra_DatePicker").settings.months;C=!1;b.each(A,function(a,b){if(C)return!0;if(segments[c+1].toLowerCase()==b.substring(0,d.character=="D"||d.character=="M"?3:b.length).toLowerCase()){switch(d.character){case "D":segments[c+1]=v[a].substring(0,3);break;case "l":segments[c+1]=v[a];break;case "F":segments[c+
1]=y[a];r=a+1;break;case "M":segments[c+1]=y[a].substring(0,3),r=a+1}C=!0}});break;case "Y":t=parseInt(segments[c+1],10);break;case "y":t="19"+parseInt(segments[c+1],10)}});C&&(l=new Date(t,r-1,s),l.getFullYear()==t&&l.getDate()==s&&l.getMonth()==r-1&&(a.data("timestamp",Date.parse(y[r-1]+" "+s+", "+t)),j=!0))}j||(e=!1)}}break;case "datecompare":switch(i.type){case "password":case "text":case "textarea":if(void 0!=d.rules[f][0]&&void 0!=d.rules[f][1]&&b(d.rules[f][0])&&c.validate_control(b(d.rules[f][0]))===
!0&&void 0!=a.data("timestamp"))switch(d.rules[f][1]){case ">":e=a.data("timestamp")>b("#"+d.rules[f][0]).data("timestamp");break;case ">=":e=a.data("timestamp")>=b("#"+d.rules[f][0]).data("timestamp");break;case "<":e=a.data("timestamp")<b("#"+d.rules[f][0]).data("timestamp");break;case "<=":e=a.data("timestamp")<=b("#"+d.rules[f][0]).data("timestamp")}else e=!1}break;case "digits":switch(i.type){case "password":case "text":case "textarea":j=RegExp("^[0-9"+z(d.rules[f][0]).replace(/\s/,"\\s")+"]+$",
"ig"),b.trim(a.val())!=""&&!j.test(a.val())&&(e=!1)}break;case "email":switch(i.type){case "password":case "text":case "textarea":b.trim(a.val())!=""&&null==a.val().match(/^([a-zA-Z0-9_\-\+\~\^\{\}]+[\.]?)+@{1}([a-zA-Z0-9_\-\+\~\^\{\}]+[\.]?)+\.[A-Za-z0-9]{2,}$/)&&(e=!1)}break;case "emails":switch(i.type){case "password":case "text":case "textarea":a.val().split(",").each(function(a){b.trim(a)!=""&&null==b.trim(a).match(/^([a-zA-Z0-9_\-\+\~\^\{\}]+[\.]?)+@{1}([a-zA-Z0-9_\-\+\~\^\{\}]+[\.]?)+\.[A-Za-z0-9]{2,}$/)&&
(e=!1)})}break;case "filesize":switch(i.type){case "file":if((p=a.data("file_info"))&&(void 0==p[2]||void 0==p[3]||p[2]!=0||parseInt(p[3],10)>parseInt(d.rules[f][0],10)))e=!1}break;case "filetype":switch(i.type){case "file":if(p=a.data("file_info"))j=b.map(d.rules[f][0].split(","),function(a){return b.trim(a)}),l=!1,(p=p[0].match(/\.(.+)$/))&&p[1]&&(p=p[1]),b.inArray(p,j)>-1&&(l=!0),l||(e=!1)}break;case "float":switch(i.type){case "password":case "text":case "textarea":if(j=RegExp("^[0-9-."+z(d.rules[f][0]).replace(/\s/,
"\\s")+"]+$","ig"),b.trim(a.val())!=""&&(b.trim(a.val())=="-"||b.trim(a.val())=="."||null!=a.val().match(/\-/g)&&a.val().match(/\-/g).length>1||null!=a.val().match(/\./g)&&a.val().match(/\./g).length>1||a.val().indexOf("-")>0||!j.test(a.val())))e=!1}break;case "image":switch(i.type){case "file":(p=a.data("file_info"))&&null==p[1].match(/image\/(gif|jpeg|png|pjpeg)/i)&&(e=!1)}break;case "image_max_width":switch(i.type){case "file":(p=a.data("file_info"))&&null!=p[1].match(/image\/(gif|jpeg|png|pjpeg)/i)&&
parseInt(p[4],10)>parseInt(d.rules[f][0],10)&&(e=!1)}break;case "image_max_height":switch(i.type){case "file":(p=a.data("file_info"))&&null!=p[1].match(/image\/(gif|jpeg|png|pjpeg)/i)&&parseInt(p[5],10)>parseInt(d.rules[f][0],10)&&(e=!1)}break;case "length":switch(i.type){case "password":case "text":case "textarea":if(a.val()!=""&&void 0!=d.rules[f][0]&&a.val().length-D(a)<d.rules[f][0]||void 0!=d.rules[f][1]&&d.rules[f][1]>0&&a.val().length-D(a)>d.rules[f][1])e=!1}break;case "number":switch(i.type){case "password":case "text":case "textarea":if(j=
RegExp("^[0-9-"+z(d.rules[f][0]).replace(/\s/,"\\s")+"]+$","ig"),b.trim(a.val())!=""&&(b.trim(a.val())=="-"||null!=a.val().match(/\-/g)&&a.val().match(/\-/g).length>1||a.val().indexOf("-")>0||!j.test(a.val())))e=!1}break;case "regexp":switch(i.type){case "password":case "text":case "textarea":j=RegExp(d.rules[f][0],"g"),b.trim(a.val())!=""&&null==j.exec(a.val())&&(e=!1)}break;case "url":switch(i.type){case "text":j=RegExp(/^(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]$/gi),
b.trim(a.val())!=""&&null==j.exec(a.val())&&(e=!1)}break;case "required":switch(i.type){case "checkbox":case "radio":var B=!1;x[i.id].each(function(){this.checked&&!this.disabled&&(B=!0)});B||(e=!1);break;case "file":case "password":case "text":case "textarea":b.trim(a.val())==""&&(e=!1);break;case "select-one":if(a.hasClass("myzebra-time")&&a.get(0).selectedIndex==0)i.id=i.id.replace(/\_(hours|minutes|seconds|ampm)$/,""),e=!1;else if(a.hasClass("myzebra-other")&&a.val()=="other"&&(!b("#"+i.id+"_other").length||
b.trim(b("#"+i.id+"_other").val())=="")||a.val()==""||a.find("option:selected").attr("disabled"))e=!1;break;case "select-multiple":a.get(0).selectedIndex==-1&&(e=!1)}break;case "upload":switch(i.type){case "file":if(p=a.data("file_info"),typeof d.rules.required!="undefined"&&(!p||!p[2]||p[2]!=0))e=!1}}if(!e)h=f,d.message=c.settings.error_messages[i.id][f=="custom"?"custom_"+g:h],d.value=a.val()}}return e?!0:h};c.validate=function(){var a=!0;for(index in o){if(!a&&!c.settings.validate_all)break;f=
o[index].element;f.attr("id");if((rule_not_passed=c.validate_control(f))!==!0)a=!1}if(a&&void 0!=c.settings.onValid)return c.settings.onValid();return a};var y=function(){b.each(v,function(){var a=b(this),c=a.data("MyZebra_Form_Placeholder");a.val()==""&&!a.is(":focus")?c.css("display","block"):c.css("display","none")})},z=function(a){return a.replace(/([-.*+?^${}()|[\]\/\\])/g,"\\$1")},A=function(a){if(a.selectionStart!=null)return a.selectionStart;var b=document.selection.createRange(),c=b.duplicate();
if(a.type=="text")return 0-c.moveStart("character",-1E5);else{var d=a.value.length;c.moveToElementText(a);c.setEndPoint("StartToStart",b);return d-c.text.length}},D=function(a){var a=a.val(),b=a.length;return a.replace(/(\r\n|\r|\n)/g,"\r\n").length-b},E=function(a){if(b.browser.msie&&b.browser.version.match(/^6/)&&!a.data("shim")){var c=a.offset(),e=parseInt(a.css("zIndex"),10)-1,c=jQuery("<iframe>",{src:'javascript:document.write("")',scrolling:"no",frameborder:0,allowTransparency:"true","class":"MyZebra_Form_error_iFrameShim",
css:{zIndex:e,position:"absolute",top:c.top,left:c.left,width:a.outerWidth(),height:a.outerHeight(),filter:"progid:DXImageTransform.Microsoft.Alpha(opacity=0)",display:"block"}});b("body").append(c);a.data("shim",c)}},B=function(a){var c=b("#"+a.attr("id")+"_other");a.val()=="other"?c.css("display","block"):c.css("display","none")},u=function(a){var b=["button","input:checkbox","input:file","input:password","input:radio","input:submit","input:text","select","textarea"];for(index in b)if(a.is(b[index])){if(b[index]==
"select")return a.attr("multiple")?"select-multiple":"select-one";return b[index].replace(/input\:/,"")}};c.init()};b.fn.MyZebra_Form=function(f){return this.each(function(){var j=new b.MyZebra_Form(this,f);b(this).data("MyZebra_Form",j)})}})(jQuery);
