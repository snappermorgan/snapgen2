...
/**
*    Migrate Plugin to support deprectaed jQuery features (eg browser)
**/
 ...
 */
;(function (a, b, c) {
    // used to allow values in options that might contain characters specifc to regexes
    ...
    // http://stackoverflow.com/questions/5297856/replaceall-in-javascript-and-dollar-sign
    ...
    function r(b, c) {
        ...
        if (c.options && c.options.length) for (var g = 0, h = c.options.length; g < h; g++) {
            var j = a(c.options[g]),
                k = "dk_option_current",
                l = i;
            l = l.replace("{{ value }}", j.val());
            l = l.replace("{{ current }}", s(j.val()) === c.value ? k : "");
            l = l.replace("{{ text }}", escapeReplace(j.text()));
            e[e.length] = l;
        }
    ...
    ***************************/
    ...
    $.MyZebra_Form_Taxonomy=function(ele, params)
    {
        ...
        var esc_term=function(str) {
          ...
          //return str; //str.replace(/[^\w \d\-_]|[\f\n\r\t\v\u00A0\u2028\u2029]+/g,'');
    ...
    $.MyZebra_Form_TaxonomyHierarchicalAddNew=function(ele,params)
    {
        ...
        var addHandler=function(){
            var newtax=esc_term($.trim(tt.val()));//$.trim(tt.val().replace(/[^\w\- ]+/g,''));
                            ...
                            switch (attributes['type']) {
                                ...
                                case 'file':
                                    ...
                                    if (file_info) {
                                        ...
                                        // REMOVE the mimes.json, use allowed mimes as parameters always
                                        ...
                                        //console.log(myzebra.mimes);
                                        //console.log(allowed_file_types);
                                        // iterate through the known mime types
                                        /*$.each(myzebra.mimes, function(extension, type) {
                                        ...
                                        // check extension, as WP does
                                        var file_ext=file_info[0].match(/\.(.+)$/);
                                        if (file_ext && file_ext[1])
                                            file_ext=file_ext[1];
                                        if ($.inArray(file_ext, allowed_file_types) > -1) found = true;
