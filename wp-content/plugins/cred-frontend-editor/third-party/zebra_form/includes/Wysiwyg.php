 ...
 *  @package    Controls
 */
class MyZebra_Form_Wysiwyg extends MyZebra_Form_Control
{
    ...
    function MyZebra_Form_Wysiwyg($id, $default = '', $attributes = '')
    {
        ...
        $this->set_attributes(
            ...
            array(
                ...
                'disable_xss_filters'=>true,
                'name'      =>  $id,
                'id'        =>  preg_replace('/\[.*\]$/', '', $id),
                'rows'      =>  5,
                'cols'      =>  '80',           // used only for passing W3C validation
                'class'     =>  'myzebra-control myzebra-textarea myzebra-wysiwyg',
                'type'      =>  'textarea',
                'value'     =>  $default,
                'has_media_button'=>false
    ...
    function _toHTML()
    {
        ...
        if ($this->attributes['has_media_button'])
            $this->set_attributes(array('class'=>'myzebra-has-media-button'),false);
        $resize_button='<div class="myzebra-rich-editor-resize"></div>';   
        // apply escape (twice, removed) for textarea using cleditor, add stripslashes to avoid escaping quotes of shortcodes etc..
        return '<div class="myzebra-rich-editor-container"><textarea ' . $this->_render_attributes() . '>' . (isset($attributes['value']) ? $this->extra_xss('textarea',stripslashes($attributes['value'])) : '') . '</textarea>'.$resize_button.'</div>';
    }
}
