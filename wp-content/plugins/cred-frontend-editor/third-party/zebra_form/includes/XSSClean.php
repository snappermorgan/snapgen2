...
class XSS_Clean
{
	...
	var $_never_allowed_str =	array(
		'document.cookie'	=> '_removed_',
		'document.write'	=> '_removed_',
		'.parentNode'		=> '_removed_',
		'.innerHTML'		=> '_removed_',
		'window.location'	=> '_removed_',
		'-moz-binding'		=> '_removed_',
		'<!--'				=> '&lt;!--',
		'-->'				=> '--&gt;',
		'<![CDATA['			=> '&lt;![CDATA[',
		'<comment>'			=> '&lt;comment&gt;'
     ...
     *  version 2.1.2.
     *
     *  <i>This method is automatically run for each control when calling {@link MyZebra_Form::validate() validate()}, unless
     *  specifically disabled by {@link MyZebra_Form_Control::disable_xss_filters() disable_xss_filters()})!</i>
     *
	...
	function sanitize($str)
	{
		...
		do
		{
			...
			if (preg_match('/script|xss/i', $str))
			{
				...
				$str = preg_replace('#</*(?:script|xss).*?\>#si', '_removed_', $str);
			}
		}
		while ($original !== $str);
	...
	function entity_decode($str, $charset = NULL)
	{
		...
		if (empty($charset))
		{
			$charset ='UTF-8'; //config_item('charset');
		}
	...
	{
		return $this->entity_decode($match[0], strtoupper('UTF-8' /*config_item('charset')*/));
	}
	...
	function _do_never_allowed($str)
	{
		...
		foreach ($this->_never_allowed_regex as $regex)
		{
			...
			$str = preg_replace('#'.$regex.'#is', '_removed_', $str);
		}
