...
class MyZebra_Form_Taxonomyhierarchical extends MyZebra_Form_Control
{
    ...
    private function buildSelect($index, $level, &$childs, &$names, &$values)
    {
        $output='';
        $padding=($level*11).'px;';
        ob_start();
        if (isset($childs[$index]))
        {
            ...
            {
                ?>
                 <option style="padding-left:<?php echo $padding; ?>"value="<?php echo $this->extra_xss('attr',$tid); ?>" <?php if (isset($values[$tid])) echo 'selected="selected"'; ?>><?php echo $this->extra_xss('html',$names[$tid]); ?></option>
                <?php
                if (isset($childs[$tid]))
                    echo $this->buildSelect($tid, $level+1, $childs, $names, $values);
            }
        }
        $output=ob_get_clean();
        return $output;
            ...
            <div class='myzebra-taxonomy-hierarchical'>
                ...
                if ($this->attributes['type']=='select')
                {
                    ...
                    if ($this->attributes['single_select'])
                        echo '<select name='.$this->attributes['name'].'[]'.' class="myzebra-select myzebra-keep-original">';
                    else
                        echo '<select name='.$this->attributes['name'].'[]'.' class="myzebra-select myzebra-keep-original" multiple="multiple">';
                    echo $this->buildSelect(0, 0, $childs, $names, $this->submitted_value);
                    echo '</select>';
                    echo "</div>";
                }
                else
