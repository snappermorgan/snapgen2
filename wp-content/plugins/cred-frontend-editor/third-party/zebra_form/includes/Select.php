 ...
 *  @package    Controls
 */
class MyZebra_Form_Select extends MyZebra_Form_Control
{
     ...
     *  "<i>-&nbsp;select&nbsp;-</i>" taken from the language file - see the {@link MyZebra_Form::language() language()}
     ...
     *                                  {@link MyZebra_Form_Text textbox} control will be automatically created having the
     ...
     *  @return void
     */
    function MyZebra_Form_Select($id, $default = '', $attributes = '', $default_other = '')
    {
        ...
        $this->set_attributes(
			...
			array(
                ...
                'name'          =>  $id,
                'id'            =>  preg_replace('/\[.*\]$/', '', $id),
                'class'         =>  'myzebra-control myzebra-select',
                'options'       =>  array(),
			    'type'          =>  'select',
                'value'         =>  $default,
                'dummy'         => false,
    ...
    {
        if (isset($this->attributes['actual_options']) && !empty($this->attributes['actual_options']))
            return $this->attributes['actual_options'][$this->attributes['value']];
        else return $this->get_values();
    }
    ...
    function _toHTML()
    {
        ...
        if (isset($attributes['multiple']))
            $this->attributes['class'].=(!empty($this->attributes['class']))?" myzebra-keep-original":"myzebra-keep-original";
    ...
    function _generate(&$options, &$selected, $level = 0)
    {
        ...
        foreach ($options as $value => $caption) {
            ...
            if (is_array($caption)) {
                ...
                $content .= '
                    <optgroup label="' . str_repeat($indent, $level) . $this->extra_xss('attr',$value) . '">
                        <option disabled="disabled" class="myzebra-dummy"></option>
                    </optgroup>
                ';
            ...
            // if entry is a standard option
                ...
                if ($this->attributes['dummy']!==false && $value==$this->attributes['dummy'])
                {
                    // create the appropriate code
                    $content .= '<option value="' . $this->extra_xss('attr',$value) . '"' .' selected="selected" disabled="disabled"' . '>' .

                    // indent appropriately
                    str_repeat($indent, $level) . $this->extra_xss('html',$caption) . '</option>';
                }
                else
                {
                    // create the appropriate code
                    $content .= '<option value="' . $this->extra_xss('attr',$value) . '"' .
