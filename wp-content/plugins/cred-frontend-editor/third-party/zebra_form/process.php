...
// if we're performing a file upload
    ...
    function process() {
        ...
        $path = preg_replace(array('/\\\\/', '/\/\//'), '/', $_GET['path']);
        
        //print_r($_FILES[$controlname]);
        ...
        if (isset($_FILES[$controlname])) 
        {
            ...
            $file['name'] = is_array($_FILES[$controlname]['name'])?$_FILES[$controlname]['name'][$index]:$_FILES[$controlname]['name'];
            $file['tmp_name'] = is_array($_FILES[$controlname]['tmp_name'])?$_FILES[$controlname]['tmp_name'][$index]:$_FILES[$controlname]['tmp_name'];
            $file['type'] = is_array($_FILES[$controlname]['type'])?$_FILES[$controlname]['type'][$index]:$_FILES[$controlname]['type'];
            $file['error'] = (!file_exists($path) ? -1 : (is_array($_FILES[$controlname]['error'])?$_FILES[$controlname]['error'][$index]:$_FILES[$controlname]['error']));   // is the upload path valid?
            $file['size'] = is_array($_FILES[$controlname]['size'])?$_FILES[$controlname]['size'][$index]:$_FILES[$controlname]['size'];
        ...
        <script type="text/javascript">
            ...
            if(undefined!=f){
                f.data('MyZebra_Form').end_file_upload('<?php echo $control . '\'' . (isset($file) ? ',[\'' . addcslashes($file['name'], '\'') . '\',\'' . $file['type'] . '\',\'' . $file['error'] . '\',\'' . $file['size'] . '\',\'' . $file['width'] . '\',\'' . $file['height'] . '\']' : '')?>)
            }
        </script>
