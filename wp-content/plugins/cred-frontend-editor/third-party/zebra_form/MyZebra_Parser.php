...
*
*   adapted to PHP form MyZebra_Parser js version 
...
class MyZebra_Regex
{
    ...
    public function MyZebra_Regex($rx,$opts='')
    {
...
class MyZebra_Functions
{
    ...
    public static function Regex($rx, $opts='')  {return new MyZebra_Regex($rx, $opts);}
...
*/
    ...
    {
        if (self::$_today)
            return new MyZebra_Date(self::$_today);
        
        $today=new MyZebra_Date();
        return  $today->setDateByTimestamp();
    }
    
    public function MyZebra_Date($date=null)
    {
        if (isset($date))
            $this->setDate($date);
    }
    ...
    {
        return mktime (0, 0, 0, $this->_date['month'], $this->_date['day_of_month'], $this->_date['year'] /*[, int $is_dst = -1 ]*/);
    }
    ...
    public function format($format)
    {
        ...
        return MyZebra_DateParser::formatDate($this, $format);
    }
}

class MyZebra_DateParser
{
    ...
    public static function parseDate($date, $supposed_format) 
    {
        ...
        if ($date instanceof MyZebra_Date)
        {
        ...
        if ($str_date != '' && $supposed_format != '') 
        {
            ...
            usort($matches,array('MyZebra_DateParser','cmp'));
    ...
    public static function formatDate($date, $format, $isTimestamp=false) 
    {
        ...
        /*if (!isset($date) || !($date instanceof MyZebra_Date))
        {
            return '';
        }
...
class MyZebra_Tokenizer
{
    ...
    public static function makeToken($tok, $force_type=null/*, $debug=false*/)
    {
        ...
        switch ($force_type)
        {
            ...
            case self::$_TOKEN_TYPES['REGEX']:
                    ...
                    {
                        $token->val=new MyZebra_Regex($tok);
                    }
    ...
    public static function cloneToken($tok)
    {
        ...
        foreach ($tok as $at=>$val)
        {
            if (self::isDateInstance($tok->$at))
                $newtok->$at=new MyZebra_Date($tok->$at->getDate());
            else
                $newtok->$at=$tok->$at;
        }
        return $newtok;
    ...
    public static function isDate($pstrVal, $format, &$getDate)
    {
        ...
        return MyZebra_DateParser::isDate($pstrVal, $format, $getDate);
    }
...
class MyZebra_Parser
{
    ...
    private function _HandleFunctions($pstrTok, &$pStack, $pdtFormat, &$parrVars)
    {
        ...
        $varTmp = $pstrTok->val;
        $arrArgs = array();
        $varTerm = MyZebra_Tokenizer::$ARG_TERMINAL;
        ...
        switch ($varTmp)
        {
            case "ARRAY" :
                ...
                while (--$intCntr >= 0)
                {
                    ...
                    $arrArray=array_merge($arrArray,MyZebra_Tokenizer::toArray($varTerm->val));
                }
                $pStack->Push(MyZebra_Tokenizer::makeToken($arrArray,MyZebra_Tokenizer::$TOKEN_TYPE['ARRAY']));
                break;
            case "TODAY" :
                $pStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_DateParser::currentDate(), MyZebra_Tokenizer::$TOKEN_TYPE['DATE']));
                break;
            case "ACOS" :
            case "ASIN" :
            case "ATAN" :
            ...
            case "TAN" :
                ...
                {
                    $objTmp = MyZebra_Tokenizer::toNumber($varTerm->val);
                    if ($varTmp == "RAND")
                    {
                ...
                if ($varTmp == "ABS")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(abs($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "CHR")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(chr($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                else if ($varTmp == "COS")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(cos($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "FIX")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(floor($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "HEX")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(dechex($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                else if ($varTmp == "LOG")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(log($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "RAND")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(mt_rand($rand_min,$rand_max),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "ROUND")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(round($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "SIN")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(sin($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "SQRT")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(sqrt($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "TAN")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(tan($objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                break;
            case "STR" :
                if (count($arrArgs) < 1)
                    throw new Exception($varTmp . " requires atleast one argument!");
                ...
                if ($varTerm->isDate)
                {
                    ...
                    $pStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_DateParser::formatDate($varTerm->val, $format),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                }
                else  // just convert to string
                    $pStack->Push(MyZebra_Tokenizer::makeToken((string)$varTerm->val.'',MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                break;
            case "ASC" :
                if (count($arrArgs) > 1)
                    throw new Exception($varTmp . " requires only one argument!");
                ...
                if (!$varTerm->isStringLiteral)
                    ...
                    $pStack->Push(MyZebra_Tokenizer::makeToken(ord($varTerm->val{0}),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                break;
            case "REGEX" :
                if (count($arrArgs) < 1)
                    throw new Exception($varTmp . " requires atleast one argument!");
                ...
                $opts=MyZebra_Tokenizer::$EMPTY_STRING;
                if (count($arrArgs)==2)
                {
                    $opts = $arrArgs[0];
                    if ($opts->isVariable)
                ...
                $pStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_Functions::Regex($varTerm->val, $opts->val),MyZebra_Tokenizer::$TOKEN_TYPE['REGEX']));
                break;
            case "LCASE" :
            case "UCASE" :
            case "NUM" :
                ...
                else
                {
                    if ($varTmp == "LCASE")
                    {
                        $pStack->Push(MyZebra_Tokenizer::makeToken(strtolower($varTerm->val),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                    }
                    else if ($varTmp == "UCASE")
                    {
                        $pStack->Push(MyZebra_Tokenizer::makeToken(strtoupper($varTerm->val),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                    }
                    else if ($varTmp == "NUM")
                    {
                        $objTmp=MyZebra_Tokenizer::toNumber($varTerm->val)+0.0;
                        if (is_nan($objTmp))
                            throw new Exception($varTmp . " cannot convert [" . $varTerm->val . "] to number!");
                        $pStack->Push(MyZebra_Tokenizer::makeToken($objTmp,MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                    }
                }
                break;
            case "LEN" :
                ...
                {
                    if ($varTerm->isStringLiteral)
                        $pStack->Push(MyZebra_Tokenizer::makeToken(strlen($varTerm->val),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                    else
                        $pStack->Push(MyZebra_Tokenizer::makeToken(count($varTerm->val),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                }
                break;
            case "USER" :
                if (count($arrArgs) < 1)
                ...
                {
                    $pStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_Functions::User($varTerm->val),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                }
                break;
            case "COOKIE" :
                if (count($arrArgs) < 1)
                ...
                {
                    $pStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_Functions::Cookie($varTerm->val),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                }
                break;
            case "CONTAINS" :
                if (count($arrArgs) < 2)
                ...
                {
                    $found=MyZebra_Functions::Contains($varTerm->val, $varTerm2->val); //in_array($varTerm2->val,$varTerm->val);
                    $pStack->Push(MyZebra_Tokenizer::makeToken($found,MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                }
                break;
            case "DATE" :
                if (count($arrArgs) < 2)
                ...
                else if (!MyZebra_Tokenizer::isDate($varTerm->val, $varFormat->val, $dateobj))
                    throw new Exception($varTmp . " can not convert [" . $varTerm->val . "] to a valid date with format [" . $varFormat->val . "]!");
                else
                {
                    if (isset($dateobj['date']))
                        $pStack->Push(MyZebra_Tokenizer::makeToken($dateobj['date'],MyZebra_Tokenizer::$TOKEN_TYPE['DATE']));
                    else
            ...
            case "RIGHT" :
                ...
                $objTmp = MyZebra_Tokenizer::toNumber($arrArgs[0]->val);
                if ($varTmp == "LEFT")
                {
                    $pStack->Push(MyZebra_Tokenizer::makeToken(substr($varTerm,0, $objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                }
                else
                {
                    $pStack->Push(MyZebra_Tokenizer::makeToken(substr($varTerm,(strlen($varTerm) - $objTmp), $objTmp),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                }
                break;
            case "MID" :
            case "IIF" :
                ...
                if ($varTmp == "MID")
                {
                    ...
                    $objOp1 = MyZebra_Tokenizer::toNumber($arrArgs[1]->val);
                    $objOp2 = MyZebra_Tokenizer::toNumber($arrArgs[0]->val);
                    $pStack->Push(MyZebra_Tokenizer::makeToken(substr($varTerm,$objOp1, $objOp2-$objOp1),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                }
                else
                {
                    $varTerm = MyZebra_Tokenizer::toBoolean($arrArgs[2]->val);
                    if ($varTerm)
                        $objOp1 = $arrArgs[1];
                    else
                        $objOp1 = $arrArgs[0];
            ...
            case "MIN" :
                ...
                while (--$intCntr>=0)
                {
                    ...
                    if (!$varTerm->isArray)
                        $_arr=array_merge($_arr,MyZebra_Tokenizer::toArray(MyZebra_Tokenizer::toNumber($varTerm->val)));
                    else
                ...
                if ($varTmp == "AVG" && !empty($_arr))
                    $pStack->Push(MyZebra_Tokenizer::makeToken($objTmp/count($_arr),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else if ($varTmp == "AVG")
                    $pStack->Push(MyZebra_Tokenizer::makeToken(0,MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                else
                    $pStack->Push(MyZebra_Tokenizer::makeToken($objTmp,MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                unset($_arr);
                break;
    ...
    private function _InFixToPostFix($arrToks)
    {
        ...
        for ($intCntr = 0; $intCntr < count($arrToks); $intCntr++)
        {
            ...
            switch ($strTok->type)
            {
                case MyZebra_Tokenizer::$TOKEN_TYPE['LEFT_PAREN'] :
                    if ($myStack->Size() > 0 && $myStack->Get(0)->isFunction)
                    {
                        $arrPFix[$intIndex] = MyZebra_Tokenizer::$ARG_TERMINAL;
                        $intIndex++;
                    }
                    $myStack->Push($strTok);
                    break;
                case MyZebra_Tokenizer::$TOKEN_TYPE['RIGHT_PAREN'] :
                    $blnStart = true;
                    ...
                    if ($myStack->IsEmpty() && $blnStart)
                        throw new Exception("Unbalanced parenthesis!");
                    break;
                case MyZebra_Tokenizer::$TOKEN_TYPE['COMMA'] :
                    while (!$myStack->IsEmpty())
                    {
                ...
                case MyZebra_Tokenizer::$TOKEN_TYPE['ARITHMETIC_OP'] :
                case MyZebra_Tokenizer::$TOKEN_TYPE['LOGICAL_OP'] :
                case MyZebra_Tokenizer::$TOKEN_TYPE['COMPARISON_OP'] :
                    switch ($strTok->val)
                    {
                        /*case "-" :
                        case "+" :
                        ...
                        default:
                            if ($strTok->val=='-')
                            {
                                ...
                                {
                                    $strTok = MyZebra_Tokenizer::$UNARY_NEGATIVE;
                                }
                            }
                            if ($strTok->val=='+')
                            {
                            ...
                            $strTop = MyZebra_Tokenizer::$EMPTY_TOKEN;
                    ...
                    break;
                default :
                    if ($strTok->type!=MyZebra_Tokenizer::$TOKEN_TYPE['FUNCTION'])
                    {
                        $arrPFix[$intIndex] = $strTok;
                        $intIndex++;
                    }
    ...
    private function _AddNewVariable($varObj, &$varArr=null)
    {
        ...
        if (isset($varObj['withType']))
        {
            ...
            switch($varObj['withType'])
            {
                ...
                default:
            //cred_log('STRING');
            //cred_log(MyZebra_Tokenizer::$TOKEN_TYPE);
                    $varToken=MyZebra_Tokenizer::makeToken($varObj['val'],MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']/*, true*/);
                    break;
            }
        }
        else
    ...
    private function _clonePostFix($pf)
    {
        $newpf=array();
        for ($i=0; $i<count($pf); $i++)
            $newpf[$i]=MyZebra_Tokenizer::cloneToken($pf[$i]);
        return $newpf;
    }
    
    private function _ParseExpression()
    {
        $arrTokens = MyZebra_Tokenizer::Tokanize($this->strInFix);
        if (!isset($arrTokens))
    ...
    private function _EvaluateExpression(&$myarrPostFix, &$myvarArr)
    {
        ...
        while ($intIndex < count($myarrPostFix))
        {
            ...
            switch ($strTok->type)
            {
                case MyZebra_Tokenizer::$TOKEN_TYPE['ARG_TERMINAL'] :
                    $myStack->Push($strTok);
                    break;
                case MyZebra_Tokenizer::$TOKEN_TYPE['UNARY_NEGATIVE'] :
                    if ($myStack->IsEmpty())
                    ...
                    $dblNo = MyZebra_Tokenizer::toNumber($objOp1->val);
                    if (is_nan($dblNo))
                        throw new Exception("Not a numeric value!");
                    else
                    {
                        $dblNo = (0 - $dblNo);
                        $myStack->Push(MyZebra_Tokenizer::makeToken($dblNo,MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                    }
                    break;
                case MyZebra_Tokenizer::$TOKEN_TYPE['UNARY_NEGATION'] :
                    if ($myStack->IsEmpty())
                    ...
                    $objOp1 = MyZebra_Tokenizer::toBoolean($objOp1->val);
                    if ($objOp1 === null)
                        throw new Exception($strTok->val . " applied not on a boolean value!");
                    else
                        $myStack->Push(MyZebra_Tokenizer::makeToken(!($objOp1),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                    break;
                case MyZebra_Tokenizer::$TOKEN_TYPE['ARITHMETIC_OP'] :
                    switch($strTok->val)
                    {
                        ...
                        case "^" :
                            ...
                            $dblVal1 = MyZebra_Tokenizer::toNumber($objOp1->val);
                            $dblVal2 = MyZebra_Tokenizer::toNumber($objOp2->val);
                            if (is_nan($dblVal1) || is_nan($dblVal2))
                                throw new Exception("Either one of the operand is not a number can not perform [" . $strTok->val . "]");
                                
                            if ($strTok->val == "^")
                                $myStack->Push(MyZebra_Tokenizer::makeToken(pow($dblVal1, $dblVal2),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                            else if ($strTok->val == "*")
                                $myStack->Push(MyZebra_Tokenizer::makeToken(($dblVal1 * $dblVal2),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                            else if ($strTok->val == "/")
                                $myStack->Push(MyZebra_Tokenizer::makeToken(($dblVal1 / $dblVal2),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                            else
                                $myStack->Push(MyZebra_Tokenizer::makeToken(($dblVal1 % $dblVal2),MyZebra_Tokenizer::$TOKEN_TYPE['NUMBER']));
                        ...
                        case "-" :
                            ...
                            {
                                if ($strTok->val == "+")
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(($objOp1->val . $objOp2->val),MyZebra_Tokenizer::$TOKEN_TYPE['STRING_LITERAL']));
                                else
                    ...
                    switch($strTok->val)
                    {
                        ...
                        case "gte" :
                            ...
                            else if ($objOp1->isBoolean && $objOp2->isBoolean)
                            {
                                ...
                                {
                                    $dblVal1 = MyZebra_Tokenizer::toBoolean($objOp1->val);
                                    $dblVal2 = MyZebra_Tokenizer::toBoolean($objOp2->val);
                                }
                                else
                            ...
                            )
                            {
                                if ($strTok->val=='=' || $strTok->val=='eq')
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(($objOp2->val->test((string)$objOp1->val)),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                else
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(!($objOp2->val->test((string)$objOp1->val)),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                break;
                            }
                            ...
                            {
                                if ($strTok->val=='=' || $strTok->val=='eq')
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(($objOp1->val->test((string)$objOp2->val)),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                else
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(!($objOp1->val->test((string)$objOp2->val)),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                break;
                            }
                            ...
                            {
                                if ($strTok->val=='=' || $strTok->val=='eq')
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_Functions::Contains($objOp1->val,$objOp2->val),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                else
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(!MyZebra_Functions::Contains($objOp1->val,$objOp2->val),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                break;
                            }
                            ...
                            {
                                if ($strTok->val=='=' || $strTok->val=='eq')
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(MyZebra_Functions::Contains($objOp2->val,$objOp1->val),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                else
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(!MyZebra_Functions::Contains($objOp2->val,$objOp1->val),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                                break;
                            }
                            ...
                            if ($strTok->val == "=" || $strTok->val == "eq")
                                $myStack->Push(MyZebra_Tokenizer::makeToken(($dblVal1 == $dblVal2),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                ...
                case MyZebra_Tokenizer::$TOKEN_TYPE['LOGICAL_OP'] :
                    switch($strTok->val)
                    {
                        ...
                        case "|" :
                            ...
                            {
                                $objTmp1 = MyZebra_Tokenizer::toBoolean($objOp1->val);
                                $objTmp2 = MyZebra_Tokenizer::toBoolean($objOp2->val);
                                if ($strTok->val == "AND"  || $strTok->val == "&")
                                    $myStack->Push(MyZebra_Tokenizer::makeToken(($objTmp1 && $objTmp2),MyZebra_Tokenizer::$TOKEN_TYPE['BOOLEAN']));
                    ...
                    break;
                case MyZebra_Tokenizer::$TOKEN_TYPE['FUNCTION'] :
    ...
    public static function setParams($params)
    {
        MyZebra_Functions::setParams((array)$params);
    }
    
    public function MyZebra_Parser($exp=null)
    {
