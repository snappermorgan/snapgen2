 ...
 */     
    ...
    public function getFields($post_type,$add_default=true)
	{
        ...
        )
        {
            $_slug='post_parent';
            $ptype=$post_type_orig;
            $parents[$_slug]=array('is_parent'=>true,'data'=>array('post_type'=>$ptype,'repetitive'=>false,'options'=>array()),'id'=>$_slug,'slug'=>$_slug,'name'=>esc_js(sprintf(__('%s Parent','wp-cred'),$ptype)),'type'=>'select','description'=>esc_js(sprintf(__('Set the %s Parent','wp-cred'),$ptype)));
        }
