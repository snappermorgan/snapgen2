		...
		<p>
			...
			<span data-cred-bind="{ action: 'show', condition: '_cred[notification][notifications][<?php echo $ii; ?>][to][type] has wp_user' }">
                ...
                <input data-cred-bind="{ validate: {
                                        required: {
                                            actions: [
                                                {action: 'validationMessage', domRef: '#notification_user_required-<?php echo $ii; ?>' },
                                                {action: 'validateSection' }
                                            ]
                                        }
                                    }, event: 'init', action: {suggest: {url: '<?php echo CRED_CRED::route('/Forms/suggestUserMail'); ?>', param: 'user', loader: '#cred_notification_user_mail_suggest_loader_<?php echo $ii; ?>'}} }" class="cred_mail_to_user" style="width:200px" name="_cred[notification][notifications][<?php echo $ii; ?>][to][wp_user][user]" placeholder="<?php echo esc_attr(__('-- Choose user --','wp-cred')); ?>" value="<?php if (isset($notification['to']['wp_user']['user'])) echo $notification['to']['wp_user']['user']; ?>"/>
	...
	<fieldset class="cred-fieldset">
        ...
        <p class="cred-label-holder">
            <label>
				...
				<input type="text" id="credmailsubject<?php echo $ii ?>" style="position:relative;width:95%;" name="_cred[notification][notifications][<?php echo $ii; ?>][mail][subject]" value="<?php echo $notification['mail']['subject']; ?>" />
			</label>
		</p>
	</fieldset>
