...
final class CRED_Helper
{
    ...
    public static function onAdminEnqueueScripts()
    {
        ...
        if ( self::$currentPage->isPostEdit || self::$currentPage->isPostNew || self::$currentPage->isCustomAdminPage )
        {
            ...
            else
            {
                if ( self::$currentPage->isCustomPostEdit || self::$currentPage->isCustomPostNew )
                {
                    CRED_Loader::loadAsset('SCRIPT/cred_cred', 'cred_cred', false, CRED_CONCAT_ASSETS);
                    CRED_Loader::loadAsset('SCRIPT/cred_wizard', 'cred_wizard');
                    CRED_Loader::loadAsset('STYLE/cred_cred_style', 'cred_cred_style', false, CRED_CONCAT_ASSETS);
                }
                else
                {
                    CRED_Loader::loadAsset('SCRIPT/cred_cred_post', 'cred_cred', false, CRED_CONCAT_ASSETS);
                    CRED_Loader::loadAsset('STYLE/cred_cred_style_nocodemirror', 'cred_cred_style', false, CRED_CONCAT_ASSETS);
                }
            }
            ...
            wp_localize_script('cred_cred', 'cred_settings', array(
                ...
                'locale' => array(
                    ...
                    'content_explain_text' => __('Build the form using HTML and CRED shortcodes. Click on the <strong>Auto-Generate Form</strong> button to create the form with default fields. Use the <strong>Insert Post Fields</strong> button to add fields that belong to this post type, or <strong>Insert Generic Fields</strong> to add any other inputs.', 'wp-cred'),
        ...
        if ( self::$currentPage->isCustomPostEdit || self::$currentPage->isCustomPostNew || self::$currentPage->isCustomAdminPage )
        {
            ?><style type="text/css">
                ...
                #icon-cred-frontend-editor {
                    background: transparent url( '<?php echo CRED_ASSETS_URL ?>/images/cred_32x32_color.png' ) no-repeat 0 0;
                }
    ...
    {
        return do_shortcode( prepend_attachment( shortcode_unautop( wpautop( convert_chars( convert_smilies( wptexturize( 
                    $content 
                ) ) ) ) ) ) );
    }
    ...
    public static function addMenuItems()
    {
	    ...
	    add_menu_page($menu_label, $menu_label, CRED_CAPABILITY, $cred_index, array(__CLASS__, 'FormsMenuPage'), CRED_ASSETS_URL .'/images/cred_18x18_color.png');
        ...
        add_submenu_page($cred_index, __( 'Help','wp-cred' ), __( 'Help','wp-cred' ),CRED_CAPABILITY,'CRED_Help', array(__CLASS__, 'HelpMenuPage'));
        // allow 3rd-party menu items to be included
    ...
    public static function cred_delete_post_link($post_id=false, $text='', $action='', $class='', $style='', $message='')
    {
    ...
    public static function cred_form($form, $post_id=false)
    {
        ...
        $output = CRED_Form_Builder::getForm($form, $post_id, false);
 ...
 **/
    public static function credDeletePostLinkShortcode($atts)
    {
       ...
       return self::cred_delete_post_link($params['post'],$params['text'],$params['action'],$params['class'],$params['style'],$params['message']);
    }
