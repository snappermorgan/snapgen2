...
final class CRED_Notification_Manager
{
    ...
    public static function triggerNotifications($post_id, $data)
    {
        if (isset($data['post']))
            $post=$data['post'];
        else
    ...
    public static function sendTestNotification($form_id, $notification)
    {
        ...
        $hashSubject=CRED_Helper::strHash($_subj);
        $_subj=self::replacePlaceholders(
            cred_translate('CRED Notification Subject '.$hashSubject, $_subj, 'cred-form-'.$form_title.'-'.$form_id)
        , $data_subject);
        ...
        // provide WPML localisation
        $hashBody=CRED_Helper::strHash($_bod);
        $_bod=self::replacePlaceholders(
                cred_translate('CRED Notification Body '.$hashBody, $_bod, 'cred-form-'.$form_title.'-'.$form_id)
            , $data_body);
    ...
    public static function sendNotifications($post_id, $form_id, $notificationsToSent)
    {
        ...
        {
            // bypass if nothing
            if (!$notification || empty($notification) || !isset($notification['to']['type']))  continue;
