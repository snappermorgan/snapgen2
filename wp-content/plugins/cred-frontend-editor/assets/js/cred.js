(function(window, $, settings, utils, gui, mvc, undefined) {
    ...
    var aux = {
        ...
        makeResizable : function($textarea)
        {
            ...
            var resizeCodemirror=function()
            {
                ...
                {
                    CodeMirrorEditors[id].setSize(/*null*/$textarea.width(), $textarea.height());
                }
            };
        ...
        toggleCodeMirror : function(on)
        {
            ...
            if (window.CodeMirror)
            {
                ...
                else if (on && !CodeMirrorEditors['content'])
                {
                    ...
                    CodeMirrorEditors['content'].setSize( $('#content').width(), height );
        ...
        genScaffold : function()
        {
            ...
            for (var f in resp.groups)
            {
                if (resp.groups.hasOwnProperty(f))
                {
                    ...
                    groups[f]=fields;
                    fields=fields.split(',');
                    groups_out+=aux.groupOutput(f,fields,resp.custom_fields, form_id_1, form_name_1, includeWPML, PAD)+NL;
                }
            }
        ...
        fieldOutput : function(field, form_id, form_name, WPML, pad)
        {
            ...
            if (field)
            {
                if (WPML)
                {
                    ...
                    field_out.push(pad+PAD+'<div class="cred-label">[wpml-string context="cred-form-'+form_name+'-'+form_id+'" name="'+field.name+'"]'+field.name+'[/wpml-string]</div>');
                }
                else
                {
                    field_out.push(pad+'<div class="cred-field cred-field-'+field.slug+'">');
                    field_out.push(pad+PAD+'<div class="cred-label">'+field.name+'</div>');
                }
                var args=[field];
    ...
    var self = {
        ...
        getFieldData : function() { return field_data; },

        getCodeMirror : function() { return CodeMirrorEditors['content']; },

        getCSSCodeMirror : function() { return CodeMirrorEditors['cred-extra-css-editor']; },

        getJSCodeMirror : function() { return CodeMirrorEditors['cred-extra-js-editor']; },
        
        getModel : function() {return _credModel},
        getView : function() {return credView},
        ...
        forms : function(useCodeMirror)
        {
            ...
            credView=new mvc.View('cred', _credModel, {
                ...
                init: function() {
                                    ...
                                    if (window.tinyMCEPreInit)
                                    {
                                        ...
                                        if (typeof(window.tinyMCE) == 'object' && window.tinyMCEPreInit.mceInit['credmailbody__i__'])
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                try { var editor = new window.tinyMCE.Editor(newId, window.tinyMCEPreInit.mceInit[newId]).render(); } catch(e){}
                                                var DOM = window.tinyMCE.DOM;
                                                DOM.events.add( DOM.select('#wp-'+newId+'-wrap'), 'mousedown', function(e){
                                                    if ( this.id )
                                        ...
                                        if ( window.tinyMCEPreInit.qtInit && window.tinyMCEPreInit.qtInit['credmailbody__i__'] ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (typeof(window.tinyMCE) != 'object')
                                                {
                                                    ...
                                                    document.getElementById('wp-'+el+'-wrap').onmousedown = function(){ window.wpActiveEditor = window.wpcfActiveEditor = this.id.slice(3, -5); }
                                                }
                    ...
                    {
                        if (!field_data || !field_data.post_fields) return;
                    ...
                    utils.attach('cred.wizardFinished', function(){
						...
						for(codemirror in CodeMirrorEditors) {
							if (codemirror == 'content') {
								...
								if (!$_metabox.hasClass('closed') && 'none'!=$_metabox.css('display'))
									CodeMirrorEditors[codemirror].setSize( $('#' + codemirror).width(), $('#' + codemirror).height() );
							}
						}
                    });
                    ...
                    $_post
                        ...
                        .on('click', '.cred_field_add_code', function(e){
                            ...
                            setTimeout(function(){$el.closest('.cred-popup-box').__hide();$el.closest('.cred-icon-button').css('z-index',1);},50);
                            return false;
                        })
